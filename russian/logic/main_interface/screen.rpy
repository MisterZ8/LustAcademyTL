# TODO: Translation updated at 2022-01-25 10:33

# game/logic/main_interface/screen.rpy:2254
translate russian parents_room_cupboard_1_label_95946a00:

    # ch_Name "{i}(At least I know they're not trying to kick me out to make room for a new baby...){/i}"
    ch_Name "{i}(По крайней мере они пытаются выставить меня из дома не для того, чтобы завести нового ребенка...){/i}"

# game/logic/main_interface/screen.rpy:2258
translate russian parents_room_cupboard_1_label_df1f2ac7:

    # ch_Name "{i}(These files are probably older than me. Nothing intresting.){/i}"
    ch_Name "{i}(Эти папки скорее всего старше чем я. Ничего интересного..){/i}"

# game/logic/main_interface/screen.rpy:2262
translate russian parents_room_cupboard_1_label_80034104:

    # ch_Name "{i}(Never saw any of them read a single book, so It's probably an organizer...){/i}"
    ch_Name "{i}(Никогда не видел их читающими книгу, так что это, вероятно, ежедневник...){/i}"

# game/logic/main_interface/screen.rpy:2284
translate russian parents_room_cupboard_2_label_e95da604:

    # ch_Name "{i}(To take or not to take, that is the question...){/i}"
    ch_Name "{i}(Брать или не брать, вот в чем вопрос...){/i}"

# game/logic/main_interface/screen.rpy:2288
translate russian parents_room_cupboard_2_label_48d20053:

    # ch_Name "{i}(Bingo! There goes the initial rent for the room.){/i}"
    ch_Name "{i}(Бинго! А вот и первый взнос за комнату.){/i}"

# game/logic/main_interface/screen.rpy:2298
translate russian parents_room_cupboard_2_label_d0276405:

    # ch_Name "{i}(Never undestood why Olivia needs to keep so much in her purse...){/i}"
    ch_Name "{i}(Никогда не понимал, зачем Olivia нужно так много всего ее в сумке...){/i}"

# game/logic/main_interface/screen.rpy:2299
translate russian parents_room_cupboard_2_label_51be7498:

    # ch_Name "{i}(Considering the fact, that she barely leaves the house...){/i}"
    ch_Name "{i}(Учитывая, что она редко выходит из дома...){/i}"

# game/logic/main_interface/screen.rpy:2309
translate russian parents_room_photo_label_6bfb4140:

    # ch_Name "{i}(Photo of Ashley from her last competition...){/i}"
    ch_Name "{i}(Фото Ashley с ее последнего соревнования...){/i}"

# game/logic/main_interface/screen.rpy:2311
translate russian parents_room_photo_label_87645085:

    # ch_Name "{i}(She's so flexible!){/i}"
    ch_Name "{i}(Она такая гибкая!){/i}"

# game/logic/main_interface/screen.rpy:2313
translate russian parents_room_photo_label_e21e4cd8:

    # ch_Name "{i}(I wonder where else she can put this agility in use...){/i}"
    ch_Name "{i}(Интересно, где еще может пригодиться эта ее способность...){/i}"

# game/logic/main_interface/screen.rpy:2321
translate russian parents_room_painting_label_4cff181a:

    # ch_Name "{i}(Don's favourite painting.){/i}"
    ch_Name "{i}(Любимая картина Дона.){/i}"

# game/logic/main_interface/screen.rpy:2323
translate russian parents_room_painting_label_3becb43f:

    # ch_Name "{i}(He is very proud that his ancestors were among the first colonists.){/i}"
    ch_Name "{i}(Он очень гордится тем, что его предки были из числа первых колонистов.){/i}"

# game/logic/main_interface/screen.rpy:2325
translate russian parents_room_painting_label_0fe9bb40:

    # ch_Name "{i}(I remember him boasting that the man on the white horse was his great-great-great grandfather.){/i}"
    ch_Name "{i}(Я помню, как он хвастался, что мужчина на белом коне - его пра-пра-прадед.){/i}"

# game/logic/main_interface/screen.rpy:2327
translate russian parents_room_painting_label_07e397eb:

    # ch_Name "{i}(Scumbag never heard of googling by the picture.){/i}"
    ch_Name "{i}(Подонок никогда не слышал о возможности поиска по картинке.){/i}"

# game/logic/main_interface/screen.rpy:2329
translate russian parents_room_painting_label_fa2857d2:

    # ch_Name "{i}(The painting was sold on Craigslist for $15 few years ago.){/i}"
    ch_Name "i}(Эта картина была продана на Craigslist за $15 несколько лет назад.){/i}"

# game/logic/main_interface/screen.rpy:2331
translate russian parents_room_painting_label_fa82a002:

    # ch_Name "{i}(I wonder what else he lied about?){/i}"
    ch_Name "{i}(Интересно о чем еще он солгал?){/i}"

# game/logic/main_interface/screen.rpy:2339
translate russian bath_breth_label_2e96eb7f:

    # ch_Name "{i}(Ashley seems to have forgotten to hide her toothbrush.){/i}"
    ch_Name "{i}(Похоже, Ashley забыла убрать свою зубную щетку.){/i}"

# game/logic/main_interface/screen.rpy:2340
translate russian bath_breth_label_7f1dded4:

    # ch_Name "{i}(She got the whole sink dirty... Olivia will get mad.){/i}"
    ch_Name "{i}(Она измазала всю раковину... Olivia будет в бешенстве.){/i}"

# game/logic/main_interface/screen.rpy:2350
translate russian bath_duck_label_113bcc9f:

    # ch_Name "{i}(Samantha and Ashley's ducks.){/i}"
    ch_Name "{i}(Уточки Samantha и Ashley's.){/i}"

# game/logic/main_interface/screen.rpy:2351
translate russian bath_duck_label_58847a7b:

    # ch_Name "{i}(No one's bathed with them in 100 years, but I hate to throw them away.){/i}"
    ch_Name "{i}(Никто не купался с ними уже 100 лет, но мне жаль их выбрасывать.){/i}"

# game/logic/main_interface/screen.rpy:2352
translate russian bath_duck_label_4e316d63:

    # ch_Name "{i}(And it's kind of cozy with them here...){/i}"
    ch_Name "{i}(С ними здесь как-то уютнее...){/i}"

# game/logic/main_interface/screen.rpy:2380
translate russian picture_in_mc_room_1f195c4f:

    # ch_Name "{i}(Is it just me, or was that picture moving?){/i}"
    ch_Name "{i}(Мне показалось или изображение двигалось?){/i}"

# game/logic/main_interface/screen.rpy:2381
translate russian picture_in_mc_room_94397271:

    # ch_Name "{i}(Hmm, i'll have to take a closer look...){/i}"
    ch_Name "{i}(Хмм, я должен подойти поближе...){/i}"

# game/logic/main_interface/screen.rpy:2382
translate russian picture_in_mc_room_c8971138:

    # ch_Name "{i}(It moved! Did you see it?){/i}"
    ch_Name "{i}(Оно двигалось! Вы это видели?){/i}"

# game/logic/main_interface/screen.rpy:2383
translate russian picture_in_mc_room_2ab96242:

    # ch_Name "{i}(Who am I talking to anyway, i'm here alone...){/i}"
    ch_Name "{i}(С кем я вообще говорю, я же здесь один...){/i}"

# game/logic/main_interface/screen.rpy:2386
translate russian picture_in_mc_room_129fdcd2:

    # ch_Name "{i}(I feel strange vibrations coming from this painting.){/i}"
    ch_Name "{i}(Я чувствую странную вибрацию, исходящую от этого изображения.){/i}"

# game/logic/main_interface/screen.rpy:2387
translate russian picture_in_mc_room_fae662e4:

    # ch_Name "{i}(I wonder who could know anything about it?){/i}"
    ch_Name "{i}(Интересно, кто может что-либо знать об этом?){/i}"

# game/logic/main_interface/screen.rpy:2388
translate russian picture_in_mc_room_763685df:

    # ch_Name "{i}(I could have sworn I heard quiet laughter...){/i}"
    ch_Name "{i}(Я готов поклясться что слышал тихий смех...){/i}"

# game/logic/main_interface/screen.rpy:2389
translate russian picture_in_mc_room_8cc9cd6b:

    # ch_Name "{i}(This painting is driving me crazy!){/i}"
    ch_Name "{i}(Это изображение сводит меня с ума!){/i}"

# game/logic/main_interface/screen.rpy:2392
translate russian picture_in_mc_room_d66ad9f9:

    # ch_Name "{i}(Sometimes I feel as if dim light is coming from that painting...){/i}"
    ch_Name "{i}(Иногда мне кажется, словно рисунок испускает слабое свечение...){/i}"

# game/logic/main_interface/screen.rpy:2393
translate russian picture_in_mc_room_1febd516:

    # ch_Name "{i}(Am I going crazy?){/i}"
    ch_Name "{i}(Я схожу с ума?){/i}"

# game/logic/main_interface/screen.rpy:2394
translate russian picture_in_mc_room_1f195c4f_1:

    # ch_Name "{i}(Is it just me, or was that picture moving?){/i}"
    ch_Name "{i}(Мне показалось или изображение двигалось?){/i}"

# game/logic/main_interface/screen.rpy:2395
translate russian picture_in_mc_room_65efa242:

    # ch_Name "{i}(Hmm, I'll have to take a closer look...){/i}"
    ch_Name "{i}(Хмм, я должен подойти поближе...){/i}"

# game/logic/main_interface/screen.rpy:2398
translate russian picture_in_mc_room_129fdcd2_1:

    # ch_Name "{i}(I feel strange vibrations coming from this painting.){/i}"
    ch_Name "{i}(Я чувствую странную вибрацию, исходящую от этого изображения.){/i}"

# game/logic/main_interface/screen.rpy:2399
translate russian picture_in_mc_room_fae662e4_1:

    # ch_Name "{i}(I wonder who could know anything about it?){/i}"
    ch_Name "{i}(Интересно, кто может что-либо знать об этом?){/i}"

# game/logic/main_interface/screen.rpy:2400
translate russian picture_in_mc_room_c8971138_1:

    # ch_Name "{i}(It moved! Did you see it?){/i}"
    ch_Name "{i}(Оно двигалось! Вы это видели?){/i}"

# game/logic/main_interface/screen.rpy:2401
translate russian picture_in_mc_room_e26ebb63:

    # ch_Name "{i}(Who am I talking to anyway, I'm here alone...){/i}"
    ch_Name "{i}(С кем я вообще говорю, я же здесь один...){/i}"

# game/logic/main_interface/screen.rpy:2415
translate russian rune_in_mc_room_667df9d9:

    # ch_Name "{i}(What strange symbols...){/i}"
    ch_Name "{i}(Какие странные символы...){/i}"

# game/logic/main_interface/screen.rpy:2416
translate russian rune_in_mc_room_eb19aaa9:

    # ch_Name "{i}(I'm sure they're there for a reason...){/i}"
    ch_Name "{i}(Я уверен, они здесь неспроста...){/i}"

# game/logic/main_interface/screen.rpy:2417
translate russian rune_in_mc_room_5b37f023:

    # ch_Name "{i}(But how do you know what secrets they hold?){/i}"
    ch_Name "{i}(Но как узнать, что скрывается за ними?){/i}"

# game/logic/main_interface/screen.rpy:2418
translate russian rune_in_mc_room_efc4aaa4:

    # ch_Name "{i}(Mystery...){/i}"
    ch_Name "{i}(Тайна...){/i}"

# game/logic/main_interface/screen.rpy:2421
translate russian rune_in_mc_room_79d4215c:

    # ch_Name "{i}(Those symbols, I can't stop thinking about them...){/i}"
    ch_Name "{i}(Я не могу перестать думать об этих символах...){/i}"

# game/logic/main_interface/screen.rpy:2422
translate russian rune_in_mc_room_dbaa33b0:

    # ch_Name "{i}(It's some kind of runes, I quess.){/i}"
    ch_Name "{i}(Думаю, это какие-то руны.){/i}"

# game/logic/main_interface/screen.rpy:2423
translate russian rune_in_mc_room_fcf17fe8:

    # ch_Name "{i}(But what are they for?){/i}"
    ch_Name "{i}(Но для чего они?){/i}"

# game/logic/main_interface/screen.rpy:2424
translate russian rune_in_mc_room_efc4aaa4_1:

    # ch_Name "{i}(Mystery...){/i}"
    ch_Name "{i}(Тайна...){/i}"

# game/logic/main_interface/screen.rpy:2427
translate russian rune_in_mc_room_d1183aca:

    # ch_Name "{i}(How stupid would it be, if it's just a decoration.){/i}"
    ch_Name "{i}(Будет глупо, если это просто декорации.){/i}"

# game/logic/main_interface/screen.rpy:2428
translate russian rune_in_mc_room_4893fcab:

    # ch_Name "{i}(No, It can't be...){/i}"
    ch_Name "{i}(Нет, не может быть...){/i}"

# game/logic/main_interface/screen.rpy:2429
translate russian rune_in_mc_room_9fa4df80:

    # ch_Name "{i}(But what is this){/i}"
    ch_Name "{i}(Но что это){/i}"

# game/logic/main_interface/screen.rpy:2430
translate russian rune_in_mc_room_efc4aaa4_2:

    # ch_Name "{i}(Mystery...){/i}"
    ch_Name "{i}(Тайна...){/i}"

# game/logic/main_interface/screen.rpy:2433
translate russian rune_in_mc_room_74b8d741:

    # ch_Name "{i}(These strange runes...){/i}"
    ch_Name "{i}(Эти странные руны...){/i}"

# game/logic/main_interface/screen.rpy:2434
translate russian rune_in_mc_room_5ac2142d:

    # ch_Name "{i}(If only I could find out what do they do...){/i}"
    ch_Name "{i}(Если бы я только смог выяснить, что они делают...){/i}"

translate russian strings:

    # game/logic/main_interface/screen.rpy:211
    old "MORNING"
    new "УТРО"

    # game/logic/main_interface/screen.rpy:211
    old "DAY"
    new "ДЕНЬ"

    # game/logic/main_interface/screen.rpy:211
    old "EVENING"
    new "ВЕЧЕР"

    # game/logic/main_interface/screen.rpy:211
    old "NIGHT"
    new "НОЧЬ"

    # game/logic/main_interface/screen.rpy:1307
    old "Monday"
    new "Понедельник"

    # game/logic/main_interface/screen.rpy:1307
    old "Tuesday"
    new "Вторник"

    # game/logic/main_interface/screen.rpy:1307
    old "Wednesday"
    new "Среда"

    # game/logic/main_interface/screen.rpy:1307
    old "Thursday"
    new "Четверг"

    # game/logic/main_interface/screen.rpy:1307
    old "Friday"
    new "Пятница"

    # game/logic/main_interface/screen.rpy:1307
    old "Saturday"
    new "Суббота"

    # game/logic/main_interface/screen.rpy:1307
    old "Sunday"
    new "Воскресенье"

    # game/logic/main_interface/screen.rpy:1377
    old "You have successfully completed all available routes. Wait for the next release."
    new "Вы успешно завершили все доступные сцены. Дождитесь следующего обновления."

    # game/logic/main_interface/screen.rpy:2286
    old "Take"
    new "Взять"

    # game/logic/main_interface/screen.rpy:2286
    old "Don't take"
    new "Не брать"

# TODO: Translation updated at 2022-03-25 06:29

translate russian strings:

    # game/logic/main_interface/screen.rpy:1492
    old "You have successfully completed all available routes. Talk to Samantha to go on vacation."
    new ""

    # game/logic/main_interface/screen.rpy:1536
    old "Skip time"
    new ""

