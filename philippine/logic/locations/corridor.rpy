# TODO: Translation updated at 2022-01-30 19:04

# game/logic/locations/corridor.rpy:7
translate philippine corridor_label_ead3beae:

    # Ashley "Ahh… Uhm… Uh…"
    Ashley ""

# game/logic/locations/corridor.rpy:10
translate philippine corridor_label_81e64aac:

    # ch_Name "{i}(Hmm? Sounds coming from Ashley's room?!){/i}"
    ch_Name "{i}(Hmm? May mga tunog galing sa kwarto ni Ashley?!){/i}"

# game/logic/locations/corridor.rpy:13
translate philippine corridor_label_ce606a14:

    # ch_Name "{i}(She may have twisted her ankle. Maybe I should check on her?){/i}"
    ch_Name "{i}(Baka nasaktan niya ang kanyang paa. Tignan ko ba?){/i}"

# game/logic/locations/corridor.rpy:23
translate philippine corridor_label_87fd2faa:

    # ch_Name "{i}(Just to be sure she's okay.){/i}"
    ch_Name "{i}(Para alam ko na ayos lang siya.){/i}"

# game/logic/locations/corridor.rpy:26
translate philippine corridor_label_7bc2ecdf:

    # ch_Name "{i}(It's none of my business.){/i}"
    ch_Name "{i}(Wala naman akong pakialam.){/i}"

# game/logic/locations/corridor.rpy:27
translate philippine corridor_label_386a98d5:

    # ch_Name "{i}(I wonder how Olivia missed that moaning though.){/i}"
    ch_Name "{i}(Bakit hindi narinig ni Olivia ang pag-ungol na iyon?){/i}"

# game/logic/locations/corridor.rpy:28
translate philippine corridor_label_646d756b:

    # ch_Name "{i}(Why was she sho pshyched about that prank letter?){/i}"
    ch_Name "{i}(Bakit sobrang nag-aalala siya sa prank letter na iyon?){/i}"

# game/logic/locations/corridor.rpy:29
translate philippine corridor_label_48c31730:

    # ch_Name "{i}(I need to hurry up if I want to find out.){/i}"
    ch_Name "{i}(Kailangan kong magmadali kung gusto kong malaman.){/i}"

# game/logic/locations/corridor.rpy:59
translate philippine ashlie_room_label_2_ef733a22:

    # ch_Name "{i}(Jackpot!){/i}"
    ch_Name ""

# game/logic/locations/corridor.rpy:62
translate philippine ashlie_room_label_2_db639e16:

    # Ashley "Um…"
    Ashley ""

# game/logic/locations/corridor.rpy:64
translate philippine ashlie_room_label_2_8cdf4eae:

    # ch_Name "{i}(Wow, Ashley is so...){/i}"
    ch_Name "{i}(Wow, si Ashley ay...){/i}"

# game/logic/locations/corridor.rpy:66
translate philippine ashlie_room_label_2_0546887e:

    # Ashley "Uhh…"
    Ashley ""

# game/logic/locations/corridor.rpy:69
translate philippine ashlie_room_label_2_9422f2a3:

    # ch_Name "{i}(Mature...){/i}"
    ch_Name "{i}(Malaki na...){/i}"

# game/logic/locations/corridor.rpy:72
translate philippine ashlie_room_label_2_82b8f9a4:

    # Ashley "Um… umh…"
    Ashley ""

# game/logic/locations/corridor.rpy:74
translate philippine ashlie_room_label_2_344ff372:

    # Ashley "Ahh… Uhm…"
    Ashley ""

# game/logic/locations/corridor.rpy:77
translate philippine ashlie_room_label_2_9e3a9b13:

    # ch_Name "{i}(What a beautiful sight!){/i}"
    ch_Name "{i}(Napakagandang tanawin!){/i}"

# game/logic/locations/corridor.rpy:79
translate philippine ashlie_room_label_2_4bf9af45:

    # ch_Name "{i}(She tries so hard to keep her moans silent.){/i}"
    ch_Name "{i}(Pilit niyang pinipigilan ang mga halinghing niya.){/i}"

# game/logic/locations/corridor.rpy:82
translate philippine ashlie_room_label_2_85170189:

    # Ashley "Ah…"
    Ashley ""

# game/logic/locations/corridor.rpy:85
translate philippine ashlie_room_label_2_05094fd4:

    # ch_Name "{i}(The fact she can't makes it even sexier!){/i}"
    ch_Name {i}(Mas maganda pa kasi hindi niya mapigilan!){/i}"

# game/logic/locations/corridor.rpy:87
translate philippine ashlie_room_label_2_bbb5db70:

    # Ashley "Ah… ah…"
    Ashley ""

# game/logic/locations/corridor.rpy:89
translate philippine ashlie_room_label_2_d08ee751:

    # ch_Name "{i}(Poor little thing is so horny...){/i}"
    ch_Name "{i}(Napukaw talaga ang kawawang ito...){/i}"

# game/logic/locations/corridor.rpy:94
translate philippine ashlie_room_label_2_64d3cd18:

    # Ashley "Um… umh… ah…"
    Ashley ""

# game/logic/locations/corridor.rpy:96
translate philippine ashlie_room_label_2_fa49132c:

    # ch_Name "{i}(If only I knew you needed assistance with your \"training\" sessions, Ashley.){/i}"
    ch_Name "{i}(Kung alam ko lang na kailangan mo ng tulong sa iyong \"pagsasanay\", Ashley.){/i}"

# game/logic/locations/corridor.rpy:98
translate philippine ashlie_room_label_2_26c294ac:

    # ch_Name "{i}(Maybe next time...){/i}"
    ch_Name "{i}(Siguro sa susunod nalang...){/i}"

# game/logic/locations/corridor.rpy:100
translate philippine ashlie_room_label_2_884e13db:

    # Ashley "Ah… ah… ah…"
    Ashley ""

# game/logic/locations/corridor.rpy:102
translate philippine ashlie_room_label_2_3f285bf6:

    # ch_Name "{i}(She's almost there...){/i}"
    ch_Name "{i}(Malapit na siya...){/i}"

# game/logic/locations/corridor.rpy:107
translate philippine ashlie_room_label_2_959e61f6:

    # Ashley "Ah… A-Ah…"
    Ashley ""

# game/logic/locations/corridor.rpy:109
translate philippine ashlie_room_label_2_8c91a788:

    # Ashley "Mmhm... uh... yeah..."
    Ashley ""

# game/logic/locations/corridor.rpy:112
translate philippine ashlie_room_label_2_550fb4eb:

    # ch_Name "{i}(Good job, baby!){/i}"
    ch_Name "{i}(Ang galing, baby!){/i}"

# game/logic/locations/corridor.rpy:114
translate philippine ashlie_room_label_2_4f666b66:

    # ch_Name "{i}(Next time I'll show you an exercise for two!){/i}"
    ch_Name "{i}(Sa susunod ipapakita ko sa iyo ang ehersisyo para sa ating dalawa!){/i}"

# game/logic/locations/corridor.rpy:117
translate philippine ashlie_room_label_2_b90f8537:

    # ch_Name "{i}(Oh shit, I completely forgot about Olivia.){/i}"
    ch_Name "{i}(Naku, nakalimutan ko talaga si Olivia.){/i}"

# game/logic/locations/corridor.rpy:119
translate philippine ashlie_room_label_2_3aabdd38:

    # ch_Name "{i}(I need to hurry if I want to catch anything.){/i}"
    ch_Name "{i}(Kailangan kong magmadali kung may gusto akong mahuli.){/i}"

# game/logic/locations/corridor.rpy:126
translate philippine before_i_need_to_hurry_if_e22ac7b2:

    # ch_Name "{i}(Woah!){/i}"
    ch_Name ""

# game/logic/locations/corridor.rpy:127
translate philippine before_i_need_to_hurry_if_08d59acf:

    # ch_Name "{i}(Alright, I shouldn’t be seeing it.){/i}"
    ch_Name "{i}(Sige, hindi ko dapat ito nakikita.){/i}"

# game/logic/locations/corridor.rpy:128
translate philippine before_i_need_to_hurry_if_3d6da892:

    # ch_Name "{i}(I need to hurry up if I want to find out why was Olivia so pshyched!){/i}"
    ch_Name "{i}(Kailangan kong magmadali kung gusto kong malaman kung bakit nagkaganoon si Olivia!){/i}"

# game/logic/locations/corridor.rpy:139
translate philippine i_need_to_hurry_if_c888624e:

    # Olivia "Disaster! This is a disaster!"
    Olivia "Kalamidad! Ito ay isang kalamidad!"

# game/logic/locations/corridor.rpy:141
translate philippine i_need_to_hurry_if_edadeda3:

    # Olivia "It’s hard enough to deal with that boy! And now \"this\"!"
    Olivia "Ang hirap kayang magpalaki sa batang iyon! At ngayon \"ito\"!"

# game/logic/locations/corridor.rpy:143
translate philippine i_need_to_hurry_if_52d1c892:

    # Olivia "Doesn't it bother you, Don? Don't you understand what kind of situation we are in?"
    Olivia "Hindi ka ba nakakaabala, Don? Hindi mo ba naiintindihan kung ano ang sitwasyon natin?"

# game/logic/locations/corridor.rpy:145
translate philippine i_need_to_hurry_if_f5622498:

    # Don "Calm down, Olivia. Why do you think this is bad news?"
    Don "Tumahimik ka, Olivia. Bakit sa tingin mo ito ay masamang balita?"

# game/logic/locations/corridor.rpy:147
translate philippine i_need_to_hurry_if_fbfc8296:

    # Olivia "Are you not listening to me? He got a letter! This means that he is one of {i}\"them\"!{/i}"
    Olivia "Hindi ka ba nakikinig sa akin? May sulat siya! Ibig sabihin kasali siya sa {i}\"kanila\"!{/i}"

# game/logic/locations/corridor.rpy:149
translate philippine i_need_to_hurry_if_5247ea6c:

    # Olivia "How can I make him obey now?{w} He was contradicting every word I said before, but now..."
    Olivia "Paano ko siya mapapasunod ngayon?{w} Sinasalungat niya ang bawat salitang sinabi ko noon, pero..."

# game/logic/locations/corridor.rpy:151
translate philippine i_need_to_hurry_if_88c9173b:

    # Don "Now he's not your problem."
    Don "Ngayon hindi na siya problema."

# game/logic/locations/corridor.rpy:153
translate philippine i_need_to_hurry_if_b2e469d8:

    # Olivia "What do you mean by that?"
    Olivia "Ano ang ibig mong sabihin?"

# game/logic/locations/corridor.rpy:155
translate philippine i_need_to_hurry_if_408a6227:

    # Don "What I'm trying to say is that this bastard isn't our headache anymore!"
    Don "Ang gusto kong sabihin ay hindi na ang bastos na ito ang sakit ng ulo natin!"

# game/logic/locations/corridor.rpy:157
translate philippine i_need_to_hurry_if_3f38818f:

    # Don "We need to help him pack his bags and pray that they will accept him."
    Don "Kailangan nating tulungan siyang mag-impake at manalangin na tanggapin siya."

# game/logic/locations/corridor.rpy:159
translate philippine i_need_to_hurry_if_4f40f895:

    # Olivia "I don't understand you, Don."
    Olivia "Hindi kita maintindihan, Don."

# game/logic/locations/corridor.rpy:161
translate philippine i_need_to_hurry_if_cc56433a:

    # Don "If he gets accepted into the academy, he will be out of our home at least until the end of the year."
    Don "Kung tatanggapin siya sa akademya, wala siya sa ating tahanan kahit hanggang sa katapusan ng taon."

# game/logic/locations/corridor.rpy:163
translate philippine i_need_to_hurry_if_18590874:

    # Don "And in the long term, he may finally find his place in life and get off my neck."
    Don "At sa tamang panahon, makahanap siya ng kanyang lugar at mawala sa ating buhay."

# game/logic/locations/corridor.rpy:165
translate philippine i_need_to_hurry_if_afb5a8a7:

    # Don "Just be nicer to him now. Just in case…"
    Don "Maging mas mabait ka sa kanya ngayon. Kung sakali..."

# game/logic/locations/corridor.rpy:167
translate philippine i_need_to_hurry_if_2518b682:

    # Olivia "Ah, Don! You're so clever!"
    Olivia "Ah, Don! Napakatalino mo!"

# game/logic/locations/corridor.rpy:172
translate philippine i_need_to_hurry_if_1408dd6b:

    # ch_Name "{i}(Hm, I wonder why Olivia got so bothered with that stupid letter...){/i}"
    ch_Name "{i}(Hm, nagtataka ako kung bakit naabala si Olivia sa katangahang sulat na iyon...){/i}"

# game/logic/locations/corridor.rpy:174
translate philippine i_need_to_hurry_if_c5c824ec:

    # Samantha "And what are you doing here, exactly?"
    Samantha "At ano ang ginagawa mo dito?"

# game/logic/locations/corridor.rpy:178
translate philippine i_need_to_hurry_if_80b5ed7f:

    # ch_Name "Fuck!"
    ch_Name "Puta!"

# game/logic/locations/corridor.rpy:208
translate philippine i_1_051_test_180c8976:

    # ch_Name "Samantha! You can't scare people like that!"
    ch_Name "Samantha! Hindi ka pwedeng makatakot ng mga tao ng ganyan!"

# game/logic/locations/corridor.rpy:210
translate philippine i_1_051_test_5ff81acc:

    # Samantha "The last time I checked, you can't spy on people either. "
    Samantha "Kung tama ang naalala ko, hindi ka rin pwedeng mag-espasya sa mga tao."

# game/logic/locations/corridor.rpy:212
translate philippine i_1_051_test_9ba22bb6:

    # Samantha "Or did you not know that?"
    Samantha "O hindi mo ba alam iyon?"

# game/logic/locations/corridor.rpy:214
translate philippine i_1_051_test_bc04450c:

    # ch_Name "Of course I know, but I have a good reason.{w} You won't rat me out to Don, will you? I beg you!"
    ch_Name "Siyempre alam ko, pero may magandang dahilan naman.{w} Hindi mo naman ako buburahin kay Don, diba? Nakikiusap ako sa'yo!"

# game/logic/locations/corridor.rpy:216
translate philippine i_1_051_test_d1817470:

    # Samantha "Didn't even cross my mind. I hope you don't make eavesdropping a habit."
    Samantha "Hindi man lang sumagi sa isip ko. Sana huwag mong ulitin ito."

# game/logic/locations/corridor.rpy:218
translate philippine i_1_051_test_ec581336:

    # Samantha "Did you already get the letter I was telling you about?"
    Samantha "Nakuha mo na ba yung liham na sinasabi ko sayo?"

# game/logic/locations/corridor.rpy:220
translate philippine i_1_051_test_beec5d34:

    # ch_Name "Yes, but Olivia confiscated it and ran to Don. "
    ch_Name "Oo, pero kinumpiska ito ni Olivia at tumakbo papunta kay Don."

# game/logic/locations/corridor.rpy:222
translate philippine i_1_051_test_175c704c:

    # ch_Name "I'm just trying to understand what they are talking about…"
    ch_Name "Sinusubukan ko lang intindihin ang pinag-uusapan nila..."

# game/logic/locations/corridor.rpy:224
translate philippine i_1_051_test_1f2da946:

    # Samantha "And you didn't have time to read it?"
    Samantha "At wala kang oras para basahin ito?"

# game/logic/locations/corridor.rpy:226
translate philippine i_1_051_test_dc7eb86b:

    # ch_Name "I read it. And at first I was sure it was a stupid prank of yours."
    ch_Name "Nabasa ko. Naging sigurado akong kalokohan mo 'yun."

# game/logic/locations/corridor.rpy:228
translate philippine i_1_051_test_9c978051:

    # ch_Name "But your parents reacted rather strangely."
    ch_Name "Pero kakaiba ang reaksyon ng mga magulang mo."

# game/logic/locations/corridor.rpy:230
translate philippine i_1_051_test_3d73f30d:

    # Samantha "Believe me, I wouldn't go through such a hassle for the sake of some kind of prank."
    Samantha "Maniwala ka sa akin, hindi ako dadaan sa ganoong abala para sa kapakanan ng kalokohan."

# game/logic/locations/corridor.rpy:232
translate philippine i_1_051_test_d6385197:

    # ch_Name "How would you react if you got an invitation to a school of wizardry?"
    ch_Name "Ano ang magiging reaksyon mo kung nakatanggap ka ng imbitasyon sa isang paaralan ng mahika?"

# game/logic/locations/corridor.rpy:234
translate philippine i_1_051_test_2b2801cd:

    # Samantha "I understand you, I experienced it myself last year."
    Samantha "Naiintindihan kita, nakaranas ako nito noong nakaraang taon."

# game/logic/locations/corridor.rpy:236
translate philippine i_1_051_test_ec5354f0:

    # ch_Name "Hmm, I remember something like that. It wasn't a prank then?"
    ch_Name "Hmm, may naalala akong ganyan. Hindi kalokohan iyon?"

# game/logic/locations/corridor.rpy:238
translate philippine i_1_051_test_4ce32109:

    # Samantha "No, but I couldn't tell you about it."
    Samantha "Hindi, ngunit hindi ko masabi sa iyo."

# game/logic/locations/corridor.rpy:240
translate philippine i_1_051_test_4b8dbc1c:

    # Samantha "It isn't customary to discuss our secrets outside the Academy."
    Samantha "Hindi pwedeng pag-usapan ang mga sikreto sa labas ng Academy."

# game/logic/locations/corridor.rpy:242
translate philippine i_1_051_test_127d5fab:

    # ch_Name "Secret school of magic? Are you studying to become an illusionist there?"
    ch_Name "Sikretong paaralan ng mahika? Nag-aaral ka ba para maging ilusyonista doon?"

# game/logic/locations/corridor.rpy:244
translate philippine i_1_051_test_2e1ccfd0:

    # Samantha "No, not a illusionist. This is exactly what the letter says it is."
    Samantha "Hindi, hindi ilusyonista. Tulad talaga sa sinasabi ng liham."

# game/logic/locations/corridor.rpy:246
translate philippine i_1_051_test_b8d3998a:

    # ch_Name "There aren't many details in it. What is Cordale? And what does this magic look like?"
    ch_Name "Walang maraming detalye dito. Ano ang Cordale? At ano ang hitsura ng mahikang ito?"

# game/logic/locations/corridor.rpy:248
translate philippine i_1_051_test_d3942ab5:

    # Samantha "You have a lot of questions but I cannot answer them..."
    Samantha "Marami kang tanong pero hindi ko masagot..."

# game/logic/locations/corridor.rpy:250
translate philippine i_1_051_test_3fdf8efe:

    # ch_Name "But why? What kind of mystery is this?"
    ch_Name "Pero bakit? Anong klaseng misteryo ito?"

# game/logic/locations/corridor.rpy:252
translate philippine i_1_051_test_d1ea040b:

    # Samantha "It is strictly forbidden for all students to talk about the Academy outside of it."
    Samantha "Mahigpit na ipinagbabawal ang pag-usapan tungkol sa Academy sa labas."

# game/logic/locations/corridor.rpy:254
translate philippine i_1_051_test_536f52f5:

    # ch_Name "Is it really that strict?"
    ch_Name "Ganun ba talaga kahigpit?"

# game/logic/locations/corridor.rpy:256
translate philippine i_1_051_test_27035b52:

    # Samantha "If I blabber, they will turn me into a pig.{w} By the way, they'll have to turn you into one too..."
    Samantha "Kung sabihin ko, gagawin nila akong baboy.{w} Oo nga pala, ikaw rin..."

# game/logic/locations/corridor.rpy:258
translate philippine i_1_051_test_f0d125d5:

    # Samantha "Just kidding! I will simply be expelled and the path to magic will be forever closed to me."
    Samantha "Biro lang! Papatalsikin lang ako at tuluyan nang sarado sa akin ang daan patungo sa mahika."

# game/logic/locations/corridor.rpy:260
translate philippine i_1_051_test_c09b7cb1:

    # ch_Name "Wow, that sucks... Okay! Let's say I believe you. What is the plan?"
    ch_Name "Wow, ang pangit... Sige! Isipin natin na naniniwala ako. Ano ang plano?"

# game/logic/locations/corridor.rpy:262
translate philippine i_1_051_test_ca929f6f:

    # Samantha "To be on time for the exam, you need to be at the train station tomorrow at noon..."
    Samantha "Para nandoon ka para sa pagsusulit, kailangan mong pumunta sa tren bukas sa tanghali..."

# game/logic/locations/corridor.rpy:264
translate philippine i_1_051_test_819331ef:

    # ch_Name "Woah. That's right, August 31st is tomorrow..."
    ch_Name "Woah. Tama, August 31 na bukas..."

# game/logic/locations/corridor.rpy:266
translate philippine i_1_051_test_9db2befe:

    # Samantha "That's what I'm saying! Pack your bags and go to bed, I'll wake you up in the morning."
    Samantha "'Yan na nga ba ang sinasabi ko! Ihanda mo na ang mga gamit mo at matulog ka na, gigisingin kita sa umaga."

# game/logic/locations/corridor.rpy:268
translate philippine i_1_051_test_17cfcff0:

    # ch_Name "Sounds reasonable. Until tomorrow then!"
    ch_Name "Makatwiran naman. Bukas naman!"

# game/logic/locations/corridor.rpy:270
translate philippine i_1_051_test_a2ad67db:

    # ch_Name "{i}(I can't believe I'm being fooled by this...){/i}"
    ch_Name "{i}(Hindi ako makapaniwalang niloloko ako nito...){/i}"

# game/logic/locations/corridor.rpy:272
translate philippine i_1_051_test_fc3704ae:

    # ch_Name "{i}(If this is a prank, then it is already going too far.){/i}"
    ch_Name {i}(Kung isa itong kalokohan, malalayo na ito.){/i}"

# game/logic/locations/corridor.rpy:274
translate philippine i_1_051_test_d10a117e:

    # ch_Name "Sam!"
    ch_Name ""

# game/logic/locations/corridor.rpy:278
translate philippine i_1_051_test_45d094f8:

    # Samantha "What, [Name]?"
    Samantha "Ano, [Name]?"

# game/logic/locations/corridor.rpy:280
translate philippine i_1_051_test_98101a54:

    # ch_Name "Swear you're not messing with me."
    ch_Name "Ipangako mo sa akin na hindi ka nagbibiro.."

# game/logic/locations/corridor.rpy:282
translate philippine i_1_051_test_1fba17d9:

    # Samantha "I swear."
    Samantha "Nangangako ako."

# game/logic/locations/corridor.rpy:284
translate philippine i_1_051_test_1ca92a70:

    # ch_Name "Okay. Good night, Samantha."
    ch_Name "Sige. Magandang gabi, Samantha."

# game/logic/locations/corridor.rpy:286
translate philippine i_1_051_test_c5716951:

    # ch_Name "{i}(I wonder what's going on with Olivia and Don.){/i}"
    ch_Name "{i}(Nagtataka ako kung ano ang nangyayari kina Olivia at Don.){/i}"

# game/logic/locations/corridor.rpy:288
translate philippine i_1_051_test_19476bd8:

    # ch_Name "{i}(I know I've promised Sam to stop eavesdropping...){/i}"
    ch_Name "{i}(Alam kong nangako ako kay Sam na titigil ako sa pakikinig...){/i}"

# game/logic/locations/corridor.rpy:289
translate philippine i_1_051_test_836fd714:

    # ch_Name "{i}(...but I don't want to miss anything!){/i}"
    ch_Name "{i}(...pero wala akong gustong palampasin!){/i}"

# game/logic/locations/corridor.rpy:290
translate philippine i_1_051_test_81e7ad8c:

    # ch_Name "{i}(What should I do?){/i}"
    ch_Name "{i}(Ano ang dapat kong gawin?){/i}"

# game/logic/locations/corridor.rpy:295
translate philippine i_1_051_test_66f05b0e:

    # ch_Name "{i}(I guess one last time does not count!){/i}"
    ch_Name  "{i}(Wala namang problema kung huling pagkakataon na ito!){/i}"

# game/logic/locations/corridor.rpy:298
translate philippine i_1_051_test_32ed700b:

    # ch_Name "{i}(I'm a man of my word. I'd better go.){/i}"
    ch_Name "{i}(Hindi ako sinungaling. Mas mabuti pang umalis na ako.){/i}"

# game/logic/locations/corridor.rpy:319
translate philippine i_1_051_test_from_gallery_d15af015:

    # '[Name]' "{i}(Nah, I dont want to see this.){/i}"
    '[Name]' "{i}(Nah, ayaw kong makita ito.){/i}"

# game/logic/locations/corridor.rpy:324
translate philippine i_1_051_test_from_gallery_27b77f45:

    # Don "It's time to use your head as it was intented too!"
    Don "Panahon na para gamitin ang iyong ulo!"

# game/logic/locations/corridor.rpy:326
translate philippine i_1_051_test_from_gallery_3a9086a1:

    # Don "Because your chattering is giving me headache."
    Don "Nakakasakit kasi ng ulo ang kadaldalan mo."

# game/logic/locations/corridor.rpy:328
translate philippine i_1_051_test_from_gallery_7608bb75:

    # Olivia "I'm..."
    Olivia "Ako ay..."

# game/logic/locations/corridor.rpy:330
translate philippine i_1_051_test_from_gallery_e5d596b8:

    # Don "What are you saying? I can't hear you!"
    Don "Ano ang sinasabi mo? Hindi kita marinig!"

# game/logic/locations/corridor.rpy:334
translate philippine i_1_051_test_from_gallery_a668428e:

    # Olivia "Mhghm..."
    Olivia ""

# game/logic/locations/corridor.rpy:337
translate philippine i_1_051_test_from_gallery_8da9a5b1:

    # Don "Yeah bitch, do you say that you like the taste of my cock?"
    Don "Oo nga, sinasabi mo ba na gusto mo ang lasa ng aking titi?"

# game/logic/locations/corridor.rpy:340
translate philippine i_1_051_test_from_gallery_a668428e_1:

    # Olivia "Mhghm..."
    Olivia ""

# game/logic/locations/corridor.rpy:342
translate philippine i_1_051_test_from_gallery_557e9d67:

    # Don "That's not all!"
    Don "Hindi iyan ang lahat!"

# game/logic/locations/corridor.rpy:344
translate philippine i_1_051_test_from_gallery_87404d4c:

    # Don "Open your mouth wider!"
    Don "Buksan mo pa ang bibig mo!"

# game/logic/locations/corridor.rpy:347
translate philippine i_1_051_test_from_gallery_55af9713:

    # Don "That's it! Eat it!"
    Don "Ayan! Kainin mo!"

# game/logic/locations/corridor.rpy:353
translate philippine i_1_051_test_from_gallery_7b58d97e:

    # Don "Yes! Yes!"
    Don "Oo! Oo!"

# game/logic/locations/corridor.rpy:357
translate philippine i_1_051_test_from_gallery_2af69542:

    # Don "Show me your boobs! Oh yeah!"
    Don "Ipakita mo sa akin ang iyong suso! Ayan!"

# game/logic/locations/corridor.rpy:367
translate philippine i_1_051_test_from_gallery_1bad80b1:

    # ch_Name "{i}(Hmm. Olivia looks unsatisfied.){/i}"
    ch_Name "{i}(Hmm. Mukhang hindi nasisiyahan si Olivia.){/i}"

# game/logic/locations/corridor.rpy:369
translate philippine i_1_051_test_from_gallery_b262ccf8:

    # Don "I'm going to take a shower, clean yourself up."
    Don "Maliligo na ako, maglinis ka na."

# game/logic/locations/corridor.rpy:371
translate philippine i_1_051_test_from_gallery_7f0478cb:

    # ch_Name "{i}(I'd better go.){/i}"
    ch_Name "{i}(Aalis na ako.){/i}"

# game/logic/locations/corridor.rpy:383
translate philippine go_to_you_room_ep1_d73a5bd6:

    # ch_Name "{i}(I can't sleep...){/i}"
    ch_Name "{i}(Hindi ako makatulog...){/i}"

# game/logic/locations/corridor.rpy:385
translate philippine go_to_you_room_ep1_27283529:

    # ch_Name "{i}(It's been a very strange day...){/i}"
    ch_Name "{i}(Napakakakaibang araw...){/i}"

# game/logic/locations/corridor.rpy:387
translate philippine go_to_you_room_ep1_042c7486:

    # ch_Name "{i}(I should check what the customer wrote about the photos...){/i}"
    ch_Name "{i}(Dapat kong tingnan kung ano ang isinulat ng customer tungkol sa mga larawan...){/i}"

# game/logic/locations/corridor.rpy:389
translate philippine go_to_you_room_ep1_df079b3e:

    # ch_Name "{i}(Though somehow that's the least of my worries right now.){/i}"
    ch_Name "{i}(Kahit na iyon ang pinakamaliit sa mga alalahanin ko ngayon.){/i}"

# game/logic/locations/corridor.rpy:391
translate philippine go_to_you_room_ep1_17dcb988:

    # ch_Name "{i}(Is all of this for real?){/i}"
    ch_Name "{i}(Totoo ba ang lahat ng ito?){/i}"

# game/logic/locations/corridor.rpy:393
translate philippine go_to_you_room_ep1_16324378:

    # ch_Name "{i}(Cordale Academy of Magic and Wizardry... Huh!){/i}"
    ch_Name ""

# game/logic/locations/corridor.rpy:395
translate philippine go_to_you_room_ep1_ca3065e0:

    # ch_Name "{i}(What kind of fool came up with that name?){/i}"
    ch_Name "{i}(Anong uri ng tanga ang nakaisip ng pangalang iyon?){/i}"

# game/logic/locations/corridor.rpy:397
translate philippine go_to_you_room_ep1_a73cc21f:

    # ch_Name "{i}(I can't wait to see Sam laughing at me in the morning.){/i}"
    ch_Name "{i}(Hindi na ako makapaghintay na makitang pinagtatawanan ako ni Sam sa umaga.){/i}"

# game/logic/locations/corridor.rpy:399
translate philippine go_to_you_room_ep1_006bbf77:

    # ch_Name "{i}(When I show up in the living room with my suitcase.){/i}"
    ch_Name "{i}(Kapag nagpakita ako sa sala na may maleta.){/i}"

# game/logic/locations/corridor.rpy:401
translate philippine go_to_you_room_ep1_97279731:

    # ch_Name "{i}(Even though she swore to me It's not a prank...){/i}"
    ch_Name "{i}(Kahit sumumpa siya sa akin na hindi ito kalokohan...){/i}"

# game/logic/locations/corridor.rpy:414
translate philippine morning_2_b6ce45a5:

    # NoName "Knock-Knock"
    NoName ""

# game/logic/locations/corridor.rpy:418
translate philippine morning_2_53b085a3:

    # ch_Name "Good morning, [Name] Junior! "
    ch_Name "Magandang umaga, [Name] Junior!"

# game/logic/locations/corridor.rpy:420
translate philippine morning_2_793f47fd:

    # ch_Name "You seem to be enjoying our dream, too."
    ch_Name "Mukhang nasisiyahan ka rin sa panaginip na ito."

# game/logic/locations/corridor.rpy:422
translate philippine morning_2_86d75a3e:

    # ch_Name "{i}(It's going to be a sunny day...){/i}"
    ch_Name "{i}(Magiging maaraw ngayong araw...){/i}"

# game/logic/locations/corridor.rpy:424
translate philippine morning_2_cba214fe:

    # ch_Name "{i}(It's a good luck!){/i}"
    ch_Name "{i}(Ang swerte naman!){/i}"

# game/logic/locations/corridor.rpy:426
translate philippine morning_2_f243bfcb:

    # ch_Name "{i}(Gotta get ready if I'm gonna play along with Samantha's joke.){/i}"
    ch_Name "{i}(Dapat maghanda ako kung makikipaglaro ako sa biro ni Samantha.){/i}"

# game/logic/locations/corridor.rpy:430
translate philippine morning_2_0f5f7b84:

    # Samantha "No thanks, I've already had breakfast, Ma."
    Samantha "Hindi na, nakapag-almusal na ako, Ma."

# game/logic/locations/corridor.rpy:432
translate philippine morning_2_5b26d4c0:

    # Samantha "We should be getting ready now. I'm just on my way to wake up [Name]!"
    Samantha "Dapat naghahanda na tayo. Pataas na nga ako para gisingin si [Name]!"

# game/logic/locations/corridor.rpy:434
translate philippine morning_2_ca8f8882:

    # Samantha "Don't worry, I won't miss the train because of him. I've got everything under control."
    Samantha "Huwag kang mag-alala, hindi ako mawawala sa tren dahil sa kanya. Kaya ko ito."

# game/logic/locations/corridor.rpy:439
translate philippine morning_2_41532e83:

    # ch_Name "Sam, where are your manners? Who taught you to barge in without knocking?"
    ch_Name "Sam, asan ang ugali mo? Sino ang nagturo sa'yong pumasok ng hindi kumakatok?"

# game/logic/locations/corridor.rpy:440
translate philippine morning_2_73a5400d:

    # Samantha "But I... Would you... Cover him up!"
    Samantha "Pero ako... Gusto mo bang... Takpan mo siya!"

# game/logic/locations/corridor.rpy:445
translate philippine morning_2_1ad81615:

    # ch_Name "Do we have to go now?"
    ch_Name "Kailangan ba nating umalis ngayon?"

# game/logic/locations/corridor.rpy:447
translate philippine morning_2_64a7eb80:

    # Samantha "..."
    Samantha ""

# game/logic/locations/corridor.rpy:449
translate philippine morning_2_d10a117e:

    # ch_Name "Sam!"
    ch_Name ""

# game/logic/locations/corridor.rpy:451
translate philippine morning_2_64a7eb80_1:

    # Samantha "..."
    Samantha ""

# game/logic/locations/corridor.rpy:453
translate philippine morning_2_09bb04bc:

    # ch_Name "Samantha!"
    ch_Name ""

# game/logic/locations/corridor.rpy:458
translate philippine morning_2_3bf47126:

    # Samantha "Admit it, did you do it on purpose?"
    Samantha "Aminin mo, sinasadya mo ba?"

# game/logic/locations/corridor.rpy:460
translate philippine morning_2_d0c304b6:

    # ch_Name "I did what on purpose? Woke up in my bed?"
    ch_Name "Ano ang sinasadya ko? Gumising?"

# game/logic/locations/corridor.rpy:462
translate philippine morning_2_7800a7fc:

    # Samantha "You met me undressed on purpose."
    Samantha ""Na nakahubad ka sa harap ko."

# game/logic/locations/corridor.rpy:464
translate philippine morning_2_4226a150:

    # ch_Name "You broke in so fast, I didn't have time to think."
    ch_Name "Ang bilis mong pumasok, wala na akong oras para mag-isip."

# game/logic/locations/corridor.rpy:466
translate philippine morning_2_2530c776:

    # ch_Name "By the way, your cheeks look nice with a blush on them!"
    ch_Name "Oo nga pala, ang ganda mo na namumula ka!"

# game/logic/locations/corridor.rpy:468
translate philippine morning_2_213ad1d3:

    # Samantha "Jerk! I got your invitation from my parents, because without it they wouldn't let you on the train."
    Samantha "Gago! Nakuha ko ang iyong imbitasyon mula sa aking mga magulang, dahil kung wala ito hindi ka nila papapasukin sa tren."

# game/logic/locations/corridor.rpy:470
translate philippine morning_2_2414603f:

    # Samantha "Hurry up and get ready, we don't have time for this."
    Samantha "Bilisan mo na at humanda ka, wala tayong oras."

# game/logic/locations/corridor.rpy:472
translate philippine morning_2_a3b159ad:

    # Samantha "I've already called a taxi. I'll meet you downstairs with your things in ten minutes."
    Samantha "I've already called a taxi. Hihintayin kita sa ibaba, may sampung minuto ka lang."

# game/logic/locations/corridor.rpy:475
translate philippine morning_2_ca98a775:

    # ch_Name "{i}(Is it just me, or could she not take her eyes off my cock?){/i}"
    ch_Name "{i}(Ako lang ba, o hindi ba niya inalis ang tingin sa aking titi?){/i}"

# game/logic/locations/corridor.rpy:477
translate philippine morning_2_05c6557c:

    # ch_Name "{i}(I'd better hurry...){/i}"
    ch_Name "{i}(Mas mabuting magmadali ako...){/i}"

# game/logic/locations/corridor.rpy:488
translate philippine morning_2_aaf32922:

    # ch_Name "{i}(Well. We're at the train station.){/i}"
    ch_Name "{i}(Nandito na kami.){/i}"

# game/logic/locations/corridor.rpy:490
translate philippine morning_2_77a99645:

    # ch_Name "{i}(Still can't believe it's true...){/i}"
    ch_Name "{i}(Hindi pa rin akong makapaniwala na totoo...){/i}"

# game/logic/locations/corridor.rpy:492
translate philippine morning_2_c297156c:

    # ch_Name "{i}(Where did Sam go?){/i}"
    ch_Name "{i}(Saan pumunta si Sam?){/i}"

# game/logic/locations/corridor.rpy:494
translate philippine morning_2_ae3642be:

    # Samantha "Oh, my God! Audrey! Stay here, I'll introduce you!"
    Samantha "Diyos ko! Audrey! Dito ka lang, ipapakilala kita!"

# game/logic/locations/corridor.rpy:496
translate philippine morning_2_bd6908b2:

    # ch_Name "{i}(Who's that?){/i}"
    ch_Name "{i}(Sino ba iyan?){/i}"

# game/logic/locations/corridor.rpy:498
translate philippine morning_2_87aeabab:

    # ch_Name "{i}(Samantha is so excited to meet her.){/i}"
    ch_Name "{i}(Nasasabik si Samantha na makita siya.){/i}"

# game/logic/locations/corridor.rpy:500
translate philippine morning_2_b7870b7d:

    # ch_Name "{i}(I've never seen her like this with any of her other girlfriends.){/i}"
    ch_Name "{i}(Hindi ko pa siya nakitang ganito kasama ang iba pa niyang kaibigan.){/i}"

# game/logic/locations/corridor.rpy:502
translate philippine morning_2_f52d109e:

    # ch_Name "{i}(Yeah, and this girl... Hot...){/i}"
    ch_Name "{i}(Oo, at ang babaeng ito... Ganda...){/i}"

# game/logic/locations/corridor.rpy:504
translate philippine morning_2_0c3cc543:

    # ch_Name "{i}(But there's something wrong with her.){/i}"
    ch_Name "{i}(Pero parang may mali sa kanya.){/i}"

# game/logic/locations/corridor.rpy:523
translate philippine i_1_072_5_test_bc3115b1:

    # Audrey "Yo. Sam, have you hooked up yet?"
    Audrey "Yo. Sam, kabit ka na ba?"

# game/logic/locations/corridor.rpy:525
translate philippine i_1_072_5_test_04f5142d:

    # ch_Name "Um, ahem, actually..."
    ch_Name "Um, ahem, sa totoo lang..."

# game/logic/locations/corridor.rpy:527
translate philippine i_1_072_5_test_7c6d364a:

    # Samantha "She's just kidding!"
    Samantha "Nagbibiro lang siya!"

# game/logic/locations/corridor.rpy:529
translate philippine i_1_072_5_test_cd42bc76:

    # Samantha "Audrey, this is my friend [Name]. Remember I told you about him?"
    Samantha "Audrey, ito si [Name]. Tandaan mo sinabi ko sayo ang tungkol sa kanya?"

# game/logic/locations/corridor.rpy:531
translate philippine i_1_072_5_test_772ccdaf:

    # Samantha "[Name], this is Audrey. My classmate and best friend."
    Samantha "[Pangalan], ito si Audrey. Kaklase at matalik kong kaibigan."

# game/logic/locations/corridor.rpy:533
translate philippine i_1_072_5_test_754b1987:

    # ch_Name "Nice to meet you."
    ch_Name "Kamusta ka."

# game/logic/locations/corridor.rpy:535
translate philippine i_1_072_5_test_c22aca09:

    # Audrey "Whatever."
    Audrey "Anuman."

# game/logic/locations/corridor.rpy:537
translate philippine i_1_072_5_test_d7e28a5b:

    # Audrey "You're cute."
    Audrey "Cute ka."

# game/logic/locations/corridor.rpy:539
translate philippine i_1_072_5_test_6894ec69:

    # Audrey "But you're cuter."
    Audrey "Pero mas cute ka."

# game/logic/locations/corridor.rpy:543
translate philippine i_1_072_5_test_d2884107:

    # ch_Name "{i}(Strange.){/i}"
    ch_Name "{i}(Kakaiba.){/i}"

# game/logic/locations/corridor.rpy:544
translate philippine i_1_072_5_test_6ff22eb8:

    # ch_Name "{i}(Why do I feel this troubling vibe around her?){/i}"
    ch_Name "{i}(Bakit nararamdaman ko itong nakakabahalang vibe sa kanya?){/i}"

# game/logic/locations/corridor.rpy:545
translate philippine i_1_072_5_test_166de9c0:

    # ch_Name "Thanks.{w} I guess."
    ch_Name "Salamat.{w} Siguro."

# game/logic/locations/corridor.rpy:548
translate philippine i_1_072_5_test_4be1cdfa:

    # Audrey "I'll get us a compartment, babe."
    Audrey "Kukuha ako ng compartment natin, babe."

# game/logic/locations/corridor.rpy:550
translate philippine i_1_072_5_test_99d16b27:

    # Audrey "Don't hang out here too long."
    Audrey "'Wag ka masyadong tumambay dito."

# game/logic/locations/corridor.rpy:552
translate philippine i_1_072_5_test_4c06122d:

    # ch_Name "And this... is your best friend?"
    ch_Name "At siya... ang kaibigan mo?"

# game/logic/locations/corridor.rpy:554
translate philippine i_1_072_5_test_e0cd4860:

    # Samantha "Oh, shut up."
    Samantha "Oh, tumahimik ka."

# game/logic/locations/corridor.rpy:557
translate philippine i_1_072_5_test_7abc94e0:

    # Samantha "She just doesn't know how to make a first impression."
    Samantha "Hindi lang siya marunong gumawa ng first impression."

# game/logic/locations/corridor.rpy:559
translate philippine i_1_072_5_test_ad1df313:

    # Samantha "You need to get to know each other better!"
    Samantha "Kailangan niyo pang makilala ang isa't isa!"

# game/logic/locations/corridor.rpy:563
translate philippine i_1_072_5_test_997d853f:

    # ch_Name "Well, we've got a whole trip to do that..."
    ch_Name "Well, pwede nating gawin sa biyahe..."

# game/logic/locations/corridor.rpy:565
translate philippine i_1_072_5_test_9528fce4:

    # ch_Name "Let's go, show me where the compartment is."
    ch_Name Tara, ituro mo sa akin kung saan ang compartment.""

# game/logic/locations/corridor.rpy:567
translate philippine i_1_072_5_test_ca8ba1e5:

    # Samantha "Hold your horses, buddy!"
    Samantha "Teka muna, 'tol!"

# game/logic/locations/corridor.rpy:569
translate philippine i_1_072_5_test_8b95c549:

    # Samantha "It's not that simple."
    Samantha "Hindi ganoon kasimple."

# game/logic/locations/corridor.rpy:571
translate philippine i_1_072_5_test_1ee458c4:

    # ch_Name "What's not so simple?"
    ch_Name "Ano ang hindi gaanong kasimple?"

# game/logic/locations/corridor.rpy:573
translate philippine i_1_072_5_test_3621634d:

    # Samantha "See, there's a separate compartment for freshmen."
    Samantha "May hiwalay na compartment para sa mga freshmen."

# game/logic/locations/corridor.rpy:575
translate philippine i_1_072_5_test_01d441c1:

    # Samantha "And I can't go in there."
    Samantha "At hindi ako makakapasok doon."

# game/logic/locations/corridor.rpy:577
translate philippine i_1_072_5_test_637c7b7e:

    # Samantha "And you can't join us."
    Samantha "At hindi ka makakasama sa amin."

# game/logic/locations/corridor.rpy:579
translate philippine i_1_072_5_test_5f14f9a8:

    # Samantha "It's academy policy."
    Samantha "Patakaran ng akademya."

# game/logic/locations/corridor.rpy:581
translate philippine i_1_072_5_test_222c4f80:

    # ch_Name "That's ridiculous."
    ch_Name "Bwisit naman."

# game/logic/locations/corridor.rpy:583
translate philippine i_1_072_5_test_66486773:

    # ch_Name "Why can't I go with you?"
    ch_Name "Bakit hindi ako makakasama?"

# game/logic/locations/corridor.rpy:585
translate philippine i_1_072_5_test_d053b0dd:

    # Samantha "So the sophomores don't tell you the secret of the entrance exam, of course."
    Samantha "Para hindi masasabi sa iyo ng mga sophomore ang sikreto ng pagsusulit."

# game/logic/locations/corridor.rpy:587
translate philippine i_1_072_5_test_3e2dc2c5:

    # ch_Name "The entrance ex..."
    ch_Name "Ang pagsus..."

# game/logic/locations/corridor.rpy:589
translate philippine i_1_072_5_test_69560604:

    # ch_Name "The entrance exam?!"
    ch_Name "Ang pagsusulit?!"

# game/logic/locations/corridor.rpy:591
translate philippine i_1_072_5_test_07ca80f0:

    # Samantha "Ah-ha-ha!"
    Samantha ""

# game/logic/locations/corridor.rpy:593
translate philippine i_1_072_5_test_023a0be5:

    # Samantha "I was expecting that reaction from you."
    Samantha "Inaasahan ko ang reaksyong iyon."

# game/logic/locations/corridor.rpy:595
translate philippine i_1_072_5_test_287cf33e:

    # ch_Name "This is funny for you?!"
    ch_Name "Ito ay nakakatawa para sa iyo?!"

# game/logic/locations/corridor.rpy:597
translate philippine i_1_072_5_test_0dee9005:

    # ch_Name "How am I supposed to pass the school of magic entrance exam, Sam?"
    ch_Name "Paano ako papasa sa pagsusulit, Sam?"

# game/logic/locations/corridor.rpy:599
translate philippine i_1_072_5_test_2776720f:

    # ch_Name "I don't even know any card tricks..."
    ch_Name "Wala akong alam na kahit anong panlilinlang..."

# game/logic/locations/corridor.rpy:601
translate philippine i_1_072_5_test_b1bf6a64:

    # Samantha "Relax, it's no big deal..."
    Samantha "Magrelax ka, hindi naman mahirap..."

# game/logic/locations/corridor.rpy:603
translate philippine i_1_072_5_test_d6c1920a:

    # Samantha "But that's all I'm gonna tell you."
    Samantha "Pero ito lang ang sasabihin ko sa iyo."

# game/logic/locations/corridor.rpy:605
translate philippine i_1_072_5_test_45f13d5f:

    # ch_Name "Samantha. Oh, please."
    ch_Name "Samantha. Pakiusap."

# game/logic/locations/corridor.rpy:607
translate philippine i_1_072_5_test_07f2259a:

    # ch_Name "I'm sure you want me to pass and study with you."
    ch_Name "Sigurado akong gusto mong pumasa ako at mag-aral kasama ka."

# game/logic/locations/corridor.rpy:609
translate philippine i_1_072_5_test_9b249ed0:

    # ch_Name "You know how much I need this chance right now..."
    ch_Name "Alam mo kung gaano ko kailangan ang pagkakataong ito ngayon..."

# game/logic/locations/corridor.rpy:611
translate philippine i_1_072_5_test_a9d6c47f:

    # ch_Name "Help me not to lose it!"
    ch_Name "Tulungan mo akong hindi mawala ito!"

# game/logic/locations/corridor.rpy:613
translate philippine i_1_072_5_test_94ba5507:

    # Samantha "Shit, I can't resist you, you know that!"
    Samantha "Bwisit, hindi kita kayang labanan, alam mo 'yun!"

# game/logic/locations/corridor.rpy:615
translate philippine i_1_072_5_test_77925122:

    # Samantha "Give me a goodbye kiss on the cheek."
    Samantha "Halikan mo ako sa pisngi."

# game/logic/locations/corridor.rpy:617
translate philippine i_1_072_5_test_1855220c:

    # ch_Name "What?"
    ch_Name "Ano?"

# game/logic/locations/corridor.rpy:619
translate philippine i_1_072_5_test_a5a9a0ac:

    # Samantha "Do as I say."
    Samantha "Gawin mo na."

# game/logic/locations/corridor.rpy:621
translate philippine i_1_072_5_test_cb98e895:

    # ch_Name "Okay."
    ch_Name "Sige."

# game/logic/locations/corridor.rpy:623
translate philippine i_1_072_5_test_8a975907:

    # Samantha "I'll meet you on the platform when you arrive."
    Samantha "Magkita tayo sa platform."

# game/logic/locations/corridor.rpy:625
translate philippine i_1_072_5_test_2a9eb020:

    # Samantha "We'll have about ten minutes before they come to meet you."
    Samantha "May sampung minuto tayo bago sila dumating upang salubungin ka."

# game/logic/locations/corridor.rpy:627
translate philippine i_1_072_5_test_f35c7f9e:

    # Samantha "So don't waste any time."
    Samantha "Kaya huwag mag-aksaya ng oras."

# game/logic/locations/corridor.rpy:629
translate philippine i_1_072_5_test_1a051f3a:

    # ch_Name "You got it."
    ch_Name "Sige."

# game/logic/locations/corridor.rpy:631
translate philippine i_1_072_5_test_011ac2ff:

    # Samantha "Oh, look, I think I see your compartment."
    Samantha "Oh, tingnan mo, parang nakikita ko na ang compartment mo."

# game/logic/locations/corridor.rpy:633
translate philippine i_1_072_5_test_d4693fdc:

    # ch_Name "Where?"
    ch_Name "Saan?"

# game/logic/locations/corridor.rpy:635
translate philippine i_1_072_5_test_f457a1b4:

    # Samantha "The second one behind you. It was listed in the letter."
    Samantha "'Yung pangalawa sa likod mo. Nakalagay sa sulat."

# game/logic/locations/corridor.rpy:637
translate philippine i_1_072_5_test_742174be:

    # ch_Name "That's great. What would I do without you, Sam?"
    ch_Name "Magaling. Ano ang gagawin ko kung wala ka, Sam?"

# game/logic/locations/corridor.rpy:639
translate philippine i_1_072_5_test_29613b59:

    # Samantha "You'd have learned how to read letters yourself?"
    Samantha "Natuto ka na bang magbasa ng mga titik?"

# game/logic/locations/corridor.rpy:641
translate philippine i_1_072_5_test_8d7bc299:

    # Samantha "You... You can read, can't you?"
    Samantha "Ikaw... Marunong ka namang magbasa diba?"

# game/logic/locations/corridor.rpy:643
translate philippine i_1_072_5_test_6fd20a6c:

    # ch_Name "Very funny."
    ch_Name "Nakakatawa."

# game/logic/locations/corridor.rpy:645
translate philippine i_1_072_5_test_c8a1dad9:

    # Samantha "Go be yourself. Such a charmer will easily make friends."
    Samantha "Huwag mong baguhin ang sarili mo. Madali kang makipagkaibigan."

# game/logic/locations/corridor.rpy:647
translate philippine i_1_072_5_test_15dbb31b:

    # ch_Name "{i}(Friends like yours?){/i}"
    ch_Name "{i}(Katulad ng mga kaibigan mo?){/i}"

# game/logic/locations/corridor.rpy:651
translate philippine i_1_072_5_test_18382b4e:

    # ch_Name "Yeah, I could use some friends."
    ch_Name "Oo, kailangan ko ng mga kaibigan."

# game/logic/locations/corridor.rpy:655
translate philippine i_1_072_5_test_8de52f6d:

    # Samantha "See you on the other side!"
    Samantha "Magkita tayo sa kabila!"

# game/logic/locations/corridor.rpy:660
translate philippine i_1_072_5_test_647af2b4:

    # ch_Name "Go on, then. Run to your girlfriend!"
    ch_Name "Sige na. tumakbo ka sa girlfriend mo!"

# game/logic/locations/corridor.rpy:666
translate philippine i_1_072_5_test_55db3660:

    # ch_Name "{i}(It wouldn't be polite not to give your beloved friend a look...){/i}"
    ch_Name "{i}(Hindi magiging magalang na hindi tingnan ang iyong minamahal na kaibigan...){/i}"

# game/logic/locations/corridor.rpy:670
translate philippine i_1_072_5_test_17cc5cb9:

    # ch_Name "{i}(Especially since she's a real eye candy.){/i}"
    ch_Name "{i}(Lalo na dahil siya'y mabuting tignan.){/i}"

# game/logic/locations/corridor.rpy:674
translate philippine i_1_072_5_test_dd3538f7:

    # ch_Name "{i}(Even her friend Audrey looked at her so predatory...){/i}"
    ch_Name "{i}(Mandaragit ang kaibigan niyang si Audrey...){/i}"

# game/logic/locations/corridor.rpy:678
translate philippine i_1_072_5_test_5179379f:

    # ch_Name "{i}(I'd give anything to know what would happen in their compartment...){/i}"
    ch_Name "{i}(Ibibigay ko ang lahat para malaman kung ano ang mangyayari sa compartment nila...){/i}"

# game/logic/locations/corridor.rpy:682
translate philippine i_1_072_5_test_8e3b37fe:

    # ch_Name "{i}(Time to find out what awaits me in mine...){/i}"
    ch_Name "{i}(Oras na para malaman kung ano ang naghihintay sa akin...){/i}"

# game/logic/locations/corridor.rpy:697
translate philippine scene_16_a87ec4e0:

    # ch_Name "{i}(Where am I heading? I still can't believe it...){/i}"
    ch_Name "{i}(Saan ako pupunta? Hindi pa rin ako makapaniwala...){/i}"

# game/logic/locations/corridor.rpy:701
translate philippine scene_16_8625d991:

    # ch_Name "{i}(Samantha was always very enthusiastic about Cordale...){/i}"
    ch_Name "{i}(Si Samantha ay palaging masigasig tungkol sa Cordale...){/i}"

# game/logic/locations/corridor.rpy:705
translate philippine scene_16_f8a9f0cd:

    # ch_Name "{i}(On the other hand, she lied that she was studying to become a lawyer. I even googled Cordale.){/i}"
    ch_Name "{i}(Sa kabilang banda, nagsinungaling siya na nag-aaral siya para maging abogado. Na-google ko ang Cordale.){/i}"

# game/logic/locations/corridor.rpy:709
translate philippine scene_16_f77030b1:

    # ch_Name "{i}(The wiki says this is an ivy league elite educational institution.){/i}"
    ch_Name "{i}(Sinasabi ng wiki na isa itong Ivy League na paaralan.){/i}"

# game/logic/locations/corridor.rpy:713
translate philippine scene_16_32aff761:

    # ch_Name "{i}(Yet there was nothing about magic... Is that true? Am I a wizard?){/i}"
    ch_Name "{i}(Gayunpaman walang tungkol sa mahika... Totoo ba iyon? Isa ba akong salamangkero?){/i}"

# game/logic/locations/corridor.rpy:716
translate philippine scene_16_280e1463:

    # UnknownHaley "Excuse me, are these seats taken?"
    UnknownHaley "Excuse me, may umuupo ba ito?"

# game/logic/locations/corridor.rpy:718
translate philippine scene_16_a1c3f1f5:

    # ch_Name "Huh?{w} What?"
    ch_Name "Huh?{w} Ano?"

# game/logic/locations/corridor.rpy:730
translate philippine i_1_082a_test_7fb91c8b:

    # ch_Name "{i}(What a cutie!){/i}"
    ch_Name "{i}(Ang cute niya!){/i}"

# game/logic/locations/corridor.rpy:732
translate philippine i_1_082a_test_975a0a11:

    # ch_Name "Oh, no, not really. Be my guest!"
    ch_Name  "Oh, wala naman. Umupo ka!"

# game/logic/locations/corridor.rpy:734
translate philippine i_1_082a_test_7c365fdd:

    # UnknownHaley "Excellent! You are my savior."
    UnknownHaley "Salamat! Ikaw ang aking tagapagligtas."

# game/logic/locations/corridor.rpy:744
translate philippine train_menu_0_c570c9df:

    # ch_Name "Let me help you!"
    ch_Name "Tutulungan kita!"

# game/logic/locations/corridor.rpy:748
translate philippine train_menu_1_98084fe6:

    # ch_Name "Here you go!"
    ch_Name "Heto!"

# game/logic/locations/corridor.rpy:750
translate philippine train_menu_1_546fcf8b:

    # UnknownHaley "Thanks."
    UnknownHaley "Salamat."

# game/logic/locations/corridor.rpy:752
translate philippine train_menu_1_91cb8934:

    # ch_Name "No problem, I'm always happy to help."
    ch_Name "Walang problema, madali naman."

# game/logic/locations/corridor.rpy:754
translate philippine train_menu_1_a90a7fa2:

    # UnknownHaley "Could you...{w} move over a little further?"
    UnknownHaley "Pwede bang...{w} lumipat pa ng kaunti?"

# game/logic/locations/corridor.rpy:756
translate philippine train_menu_1_eaf17708:

    # ch_Name "Oh. Yes, of course."
    ch_Name "Oh. Oo, Siyempre."

# game/logic/locations/corridor.rpy:769
translate philippine train_menu_2_4384efc0:

    # UnknownHaley "Hey!!! What are you staring at? You'd better help me with my suitcase."
    UnknownHaley "Hoy!!! Anong tinititigan mo? Mas mabuting tulungan mo ako sa maleta ko."

# game/logic/locations/corridor.rpy:771
translate philippine train_menu_2_4f486dbd:

    # ch_Name "This is not what it looks like. I was just distracted by... Nevermind."
    ch_Name "Hindi naman. Ginulo lang ako ng... Hindi bale."

# game/logic/locations/corridor.rpy:779
translate philippine train_menu_end_d19e87d6:

    # ch_Name "Sorry... It got awkward..."
    ch_Name "Sorry... Kakaiba ito..."

# game/logic/locations/corridor.rpy:781
translate philippine train_menu_end_49031043:

    # UnknownHaley "Don't worry, all guys are a little perverted."
    UnknownHaley "Huwag kang mag-alala, medyo bastos naman kayong mga lalaki."

# game/logic/locations/corridor.rpy:783
translate philippine train_menu_end_12663923:

    # ch_Name "{i}(Well, I would not say \"a little\"...){/i}"
    ch_Name "{i}(Well, hindi ko sasabihing \"medyo\"...){/i}"

# game/logic/locations/corridor.rpy:785
translate philippine train_menu_end_c2c689d4:

    # ch_Name "It is not true! I wasn't thinking of anything like that."
    ch_Name "Hindi totoo! Wala akong iniisip na ganyan."

# game/logic/locations/corridor.rpy:787
translate philippine train_menu_end_643c85a2:

    # UnknownHaley "Why?!"
    UnknownHaley "Bakit?!"

# game/logic/locations/corridor.rpy:789
translate philippine train_menu_end_993174e4:

    # ch_Name "Um, well..."
    ch_Name "Um, kasi..."

# game/logic/locations/corridor.rpy:791
translate philippine train_menu_end_1c912a5c:

    # UnknownHaley "Relax! I'm just kidding."
    UnknownHaley "Huwag mag-alala! Nagbibiro lang ako."

# game/logic/locations/corridor.rpy:793
translate philippine train_menu_end_9def1b56:

    # UnknownHaley "By the way, my name is Haley! Yours?"
    UnknownHaley "Nga pala, ako si Haley! Ikaw?"

# game/logic/locations/corridor.rpy:795
translate philippine train_menu_end_a34c12ad:

    # ch_Name "[Name]. Nice to meet you! You are funny."
    ch_Name "[Name]. Mabuting nakilala kita! Nakakatawa ka."

# game/logic/locations/corridor.rpy:797
translate philippine train_menu_end_048847b4:

    # Haley "Thank you, [Name]! You are nice too."
    Haley "Salamat, [Name]! Ikaw rin."

# game/logic/locations/corridor.rpy:799
translate philippine train_menu_end_6e7dc279:

    # ch_Name "Sorry if this question is a bit forward..."
    ch_Name "Pasensya na kung walang galang ang tanong na ito..."

# game/logic/locations/corridor.rpy:801
translate philippine train_menu_end_135e2748:

    # ch_Name "...but didn't all the passengers take their seats an hour ago?"
    ch_Name "...pero hindi ba naka-upo na ang ibang mga ng pasahero?"

# game/logic/locations/corridor.rpy:803
translate philippine train_menu_end_b3783af0:

    # ch_Name "How did it happen that you are coming here only now?"
    ch_Name "Bakit ngayon ka lang pumunta dito?"

# game/logic/locations/corridor.rpy:805
translate philippine train_menu_end_13e2be0c:

    # Haley "There's a rather extravagant girl in my compartment..."
    Haley "May isang medyo maluho na babae kanina..."

# game/logic/locations/corridor.rpy:807
translate philippine train_menu_end_fde91a9a:

    # Haley "She drank a bottle of whiskey in the blink of an eye and started hitting on me."
    Haley "Mabilis siyang uminom ng whisky at nagsimulang manligaw sa akin."

# game/logic/locations/corridor.rpy:809
translate philippine train_menu_end_d95d0ba4:

    # Haley "So I decided to get out of there!"
    Haley "Kaya umalis na lang ako!"

# game/logic/locations/corridor.rpy:811
translate philippine train_menu_end_30fa9be1:

    # Haley "And you are the only one who has a free seat in the couchette!"
    Haley "At ikaw lang ang walang kasama!"

# game/logic/locations/corridor.rpy:813
translate philippine train_menu_end_3f90c8fd:

    # Haley "You don't mind if I join you, do you?"
    Haley "Wala ka namang pakialam kung samahan kita, diba?"

# game/logic/locations/corridor.rpy:821
translate philippine train_menu_end_cfcc7ad5:

    # ch_Name "Of course not! There is plenty of room here, and it is always more fun with company."
    ch_Name "Siyempre! Maluwag naman dito, at mas masaya kapag may kasama."

# game/logic/locations/corridor.rpy:823
translate philippine train_menu_end_45624086:

    # Haley "Agreed! I hope you're not planning on getting drunk and harassing me?"
    Haley "Agree! Wala kang balak magpakalasing at asarin ako?"

# game/logic/locations/corridor.rpy:825
translate philippine train_menu_end_a4f16ca5:

    # ch_Name "I am not."
    ch_Name "Wala naman."

# game/logic/locations/corridor.rpy:827
translate philippine train_menu_end_5e5375c6:

    # Haley "Thanks."
    Haley "Salamat."

# game/logic/locations/corridor.rpy:829
translate philippine train_menu_end_537272fc:

    # ch_Name "For now..."
    ch_Name "Sa ngayon..."

# game/logic/locations/corridor.rpy:834
translate philippine train_menu_drunk_girls_80f77679:

    # ch_Name "Wait! Are you saying that somewhere on the train there is a compartment with a drunk hottie?"
    ch_Name "Teka! Sinasabi mo bang may lasing na bebot sa ating tren?"

# game/logic/locations/corridor.rpy:836
translate philippine train_menu_drunk_girls_058b166c:

    # Haley "Yes, couchette number 8. Why?"
    Haley "Oo, sa ika-walong karwahe. Bakit?"

# game/logic/locations/corridor.rpy:838
translate philippine train_menu_drunk_girls_6b3f7d0a:

    # ch_Name "I'll leave this compartment at your complete disposal."
    ch_Name "Iiwan ko sa iyo ang compartment na ito."

# game/logic/locations/corridor.rpy:840
translate philippine train_menu_drunk_girls_791e178a:

    # ch_Name "I have urgent business in couchette number 8!"
    ch_Name "Mayroon akong kailangang gawin sa ika-walong karwahe!"

# game/logic/locations/corridor.rpy:845
translate philippine train_menu_after_drunk_girls_2ddd6179:

    # Haley "Wait, what?!"
    Haley "Teka, ano?!"

# game/logic/locations/corridor.rpy:847
translate philippine train_menu_after_drunk_girls_cbe7ca11:

    # ch_Name "Relax, it was just a joke."
    ch_Name "Relax, biro lang 'yun."

# game/logic/locations/corridor.rpy:849
translate philippine train_menu_after_drunk_girls_b45a1a5a:

    # ch_Name "{i}(Although the prospect is not the worst.){/i}"
    ch_Name "{i}(Bagaman ang inaasam-asam ay hindi ang pinakamasama.){/i}"

# game/logic/locations/corridor.rpy:851
translate philippine train_menu_after_drunk_girls_c58f7524:

    # Haley "You are funny. I didn’t know that the Academy had a faculty of humorists."
    Haley "Nakakatawa ka. Hindi ko alam na ang Academy ay may mga komedyante."

# game/logic/locations/corridor.rpy:853
translate philippine train_menu_after_drunk_girls_57d355ce:

    # ch_Name "Someone has to take care of the future world of stand-up!"
    ch_Name "Kailangang may nag-iingat sa kinabukasan ng stand-up!"

# game/logic/locations/corridor.rpy:856
translate philippine train_menu_after_drunk_girls_97ac8811:

    # ch_Name "Besides, no stand-up works without a bit of magic!"
    ch_Name "At saka, walang stand-up na gumagana na walang mahika!"

# game/logic/locations/corridor.rpy:858
translate philippine train_menu_after_drunk_girls_94f9e592:

    # Haley "Wait, are you kidding me? Or are you serious?"
    Haley "Teka, niloloko mo ba ako? O seryoso ka?"

# game/logic/locations/corridor.rpy:860
translate philippine train_menu_after_drunk_girls_2c9c86ec:

    # Haley "What do you know about magic?"
    Haley "Ano ang alam mo sa mahika?"

# game/logic/locations/corridor.rpy:862
translate philippine train_menu_after_drunk_girls_487ed017:

    # Haley "I've been digging around the Internet and the city library all week..."
    Haley "Buong linggo akong naghuhukay sa Internet at sa aklatan..."

# game/logic/locations/corridor.rpy:864
translate philippine train_menu_after_drunk_girls_14b24ef0:

    # Haley "...but I haven't found any specifics!"
    Haley "...pero wala akong nakitang detalye!"

# game/logic/locations/corridor.rpy:866
translate philippine train_menu_after_drunk_girls_3da38669:

    # Haley "But I won't give up. I'm sure the information I found there will still be useful to me."
    Haley "Ngunit hindi ako susuko. Siguradong makinabangan ko pa rin ang impormasyong nahanap ko doon."

# game/logic/locations/corridor.rpy:868
translate philippine train_menu_after_drunk_girls_c1900c94:

    # Haley "What do you think, which people were closest to learning the theory of magic?"
    Haley "Ano sa palagay mo, sino ang pinakamalapit sa pag-aaral ng teorya ng mahika? "

# game/logic/locations/corridor.rpy:870
translate philippine train_menu_after_drunk_girls_01658f27:

    # ch_Name "Uh... Winx fairies?"
    ch_Name ""

# game/logic/locations/corridor.rpy:872
translate philippine train_menu_after_drunk_girls_bdced21f:

    # Haley "Do you know anything about magic at all?"
    Haley "May alam ka ba tungkol sa magic?"

# game/logic/locations/corridor.rpy:874
translate philippine train_menu_after_drunk_girls_0bc7dfc8:

    # ch_Name "Actually, my friend is in her sophomore year in Cordale."
    ch_Name "Actually, nasa ikalawang taon na ang kaibigan ko sa Cordale."

# game/logic/locations/corridor.rpy:876
translate philippine train_menu_after_drunk_girls_9240ebca:

    # Haley "Did she tell you anything? Please, share with me!"
    Haley "May sinabi ba siya sa'yo? Pakiusap, sabihin mo!"

# game/logic/locations/corridor.rpy:878
translate philippine train_menu_after_drunk_girls_19dffb29:

    # ch_Name "She didn't tell me anything! "
    ch_Name "Wala siyang sinabi sa akin!"

# game/logic/locations/corridor.rpy:880
translate philippine train_menu_after_drunk_girls_de19a0ac:

    # ch_Name "It is all very secret and they are forbidden to talk about the details."
    ch_Name "Sikreto ang lahat at bawal silang magsabi ng mga detalye."

# game/logic/locations/corridor.rpy:882
translate philippine train_menu_after_drunk_girls_55a87f52:

    # Haley "What friend leaves her pal in ignorance like that?"
    Haley "Sino ba ang nag-iiwan ng kanyang kaibigan sa kamangmangan?"

# game/logic/locations/corridor.rpy:884
translate philippine train_menu_after_drunk_girls_fd777908:

    # ch_Name "Don't say that! I will meet up with her when I arrive and she will help me."
    ch_Name "'Wag mong sabihin 'yan! Makikipagkita ako sa kanya pagdating natin at tutulungan niya ako."

# game/logic/locations/corridor.rpy:886
translate philippine train_menu_after_drunk_girls_05ef46dc:

    # Haley "Okay, okay! Excuse me, I got excited."
    Haley "Sige! Pasensya na, maramdamin lang ako."

# game/logic/locations/corridor.rpy:888
translate philippine train_menu_after_drunk_girls_7027103e:

    # Haley "I'm so delighted that magic exists..."
    Haley "Natutuwa akong totoo pala ang mahika..."

# game/logic/locations/corridor.rpy:890
translate philippine train_menu_after_drunk_girls_8aec3731:

    # Haley "...I can barely contain myself."
    Haley "... halos hindi ko mapigilan ang sarili ko."

# game/logic/locations/corridor.rpy:892
translate philippine train_menu_after_drunk_girls_660afcba:

    # Haley "You should know how many theories I have!"
    Haley "Gaano karaming mga teorya ang mayroon ako!"

# game/logic/locations/corridor.rpy:894
translate philippine train_menu_after_drunk_girls_fd325397:

    # Haley "What if magic is..."
    Haley "Paano kung ang mahika ay..."

# game/logic/locations/corridor.rpy:896
translate philippine train_menu_after_drunk_girls_580cca29:

    # "For the next few hours Haley shares her crazy theories about the Academy..."
    "Sa susunod na ilang oras, ibinahagi ni Haley ang kanyang mga nakatutuwang teorya tungkol sa Academy..."

# game/logic/locations/corridor.rpy:898
translate philippine train_menu_after_drunk_girls_56d523d2:

    # "...until you manage to convince her that it's worth getting some sleep before the big day."
    "...hanggang sa makumbinsi mo siya na matulog bago ang malaking araw."

# game/logic/locations/corridor.rpy:905
translate philippine train_menu_after_drunk_girls_60870614:

    # ch_Name "{i}(It was getting quite dark outside the window.){/i}"
    ch_Name "{i}(Medyo dumilim sa labas ng bintana.){/i}"

# game/logic/locations/corridor.rpy:907
translate philippine train_menu_after_drunk_girls_792b6e86:

    # ch_Name "{i}(And I still haven't fallen asleep.){/i}"
    ch_Name "{i}(At hindi pa rin ako nakatulog.){/i}"

# game/logic/locations/corridor.rpy:909
translate philippine train_menu_after_drunk_girls_92ce7869:

    # ch_Name "{i}(Time to do something about this insomnia.){/i}"
    ch_Name "{i}(Oras na para maghanap ng solusyon para sa insomnia na ito.){/i}"

# game/logic/locations/corridor.rpy:911
translate philippine train_menu_after_drunk_girls_5a292311:

    # ch_Name "{i}(But who can blame me?){/i}"
    ch_Name "{i}(Ngunit sino ang maaaring sisihin sa akin?){/i}"

# game/logic/locations/corridor.rpy:913
translate philippine train_menu_after_drunk_girls_670c78e0:

    # ch_Name "{i}(It's hard to fall asleep when your dick's on fire.){/i}"
    ch_Name "{i}(Mahirap makatulog kapag nasusunog ang titi mo.){/i}"

# game/logic/locations/corridor.rpy:915
translate philippine train_menu_after_drunk_girls_2780eec2:

    # ch_Name "{i}(I haven't fucked since I broke my leg.){/i}"
    ch_Name "{i}(Hindi pa ako nakikipagtalik simula nang mabali ang aking binti.){/i}"

# game/logic/locations/corridor.rpy:917
translate philippine train_menu_after_drunk_girls_954301ae:

    # ch_Name "{i}(And here...){/i}"
    ch_Name "{i}(At heto...){/i}"

# game/logic/locations/corridor.rpy:919
translate philippine train_menu_after_drunk_girls_72b33558:

    # ch_Name "{i}(Such a hottie!){/i}"
    ch_Name "{i}(Napakaganda!){/i}"

# game/logic/locations/corridor.rpy:921
translate philippine train_menu_after_drunk_girls_c5700282:

    # ch_Name "{i}(Flirting with me all day long.){/i}"
    ch_Name "{i}(Nililigawan ako buong araw.){/i}"

# game/logic/locations/corridor.rpy:923
translate philippine train_menu_after_drunk_girls_fd4a9606:

    # ch_Name "{i}(And now she's flashing her naked feet...){/i}"
    ch_Name "{i}(At ngayon ay pinapakislap niya ang kanyang mga hubad na paa...){/i}"

# game/logic/locations/corridor.rpy:925
translate philippine train_menu_after_drunk_girls_a58b4939:

    # ch_Name "{i}(I can't take it anymore!){/i}"
    ch_Name "{i}(Hindi ko na kaya!){/i}"

# game/logic/locations/corridor.rpy:927
translate philippine train_menu_after_drunk_girls_dc1d19db:

    # ch_Name "{i}(I need to satisfy my curiosity.){/i}"
    ch_Name "{i}(Kailangan kong makita.){/i}"

# game/logic/locations/corridor.rpy:929
translate philippine train_menu_after_drunk_girls_87ca1387:

    # ch_Name "{i}(Maybe I should take a closer look. What's the big deal?){/i}"
    ch_Name "{i}(Siguro dapat kong tingnan ng mabuti. Ano kaya ang mali doon?){/i}"

# game/logic/locations/corridor.rpy:931
translate philippine train_menu_after_drunk_girls_81e7ad8c:

    # ch_Name "{i}(What should I do?){/i}"
    ch_Name "{i}(Ano ang dapat kong gawin?){/i}"

# game/logic/locations/corridor.rpy:972
translate philippine test_choice_menu_ce8bfb54:

    # ch_Name "{i}(What have we here...){/i}"
    ch_Name "{i}(Ano ang meron dito...){/i}"

# game/logic/locations/corridor.rpy:976
translate philippine test_choice_menu_75c67607:

    # ch_Name "{i}(Well, hello there!){/i}"
    ch_Name "{i}(Kaya pala, kamusta!){/i}"

# game/logic/locations/corridor.rpy:978
translate philippine test_choice_menu_dcf27b65:

    # ch_Name "{i}(Sweet transparent panties. Nice choice for the train!){/i}"
    ch_Name "{i}(Malinaw na panloob. Maganda para sa tren!){/i}"

# game/logic/locations/corridor.rpy:982
translate philippine test_choice_menu_9cfb4ea4:

    # ch_Name "{i}(It makes me want to touch...){/i}"
    ch_Name "{i}(Nais kong hawakan...){/i}"

# game/logic/locations/corridor.rpy:1001
translate philippine test_choice_menu_9f92a07c:

    # ch_Name "{i}(Oh, yes! You're even a little wet...){/i}"
    ch_Name "{i}(Ay, oo! Medyo basa ka pala...){/i}"

# game/logic/locations/corridor.rpy:1005
translate philippine test_choice_menu_88ac51e1:

    # ch_Name "{i}(Maybe you're dreaming about me?){/i}"
    ch_Name "{i}(Baka napapanaginipan mo ako?){/i}"

# game/logic/locations/corridor.rpy:1009
translate philippine test_choice_menu_b8318c32:

    # ch_Name "{i}(Shall I take a closer look at you?){/i}"
    ch_Name "{i}(Titingnan ba kita ng malapitan?){/i}"

# game/logic/locations/corridor.rpy:1029
translate philippine test_choice_menu_7634ea46:

    # ch_Name "{i}(What a neat, tight holes you've got...){/i}"
    ch_Name "{i}(Maayos at masikip itong mga butas mo...){/i}"

# game/logic/locations/corridor.rpy:1033
translate philippine test_choice_menu_34aa4ce7:

    # ch_Name "{i}(You must be a virgin.){/i}"
    ch_Name "{i}(Siguradong birhen ka.){/i}"

# game/logic/locations/corridor.rpy:1037
translate philippine test_choice_menu_69c0fc71:

    # ch_Name "{i}(For now...){/i}"
    ch_Name "{i}(Sa ngayon...){/i}"

# game/logic/locations/corridor.rpy:1056
translate philippine train_not_too_big_32f2004a:

    # ch_Name "{i}(Not too big.){/i}"
    ch_Name "{i}(Hindi masyadong malaki.){/i}"

# game/logic/locations/corridor.rpy:1060
translate philippine train_not_too_big_2ae1076d:

    # ch_Name "{i}(But not too flat either.){/i}"
    ch_Name "{i}(Pero hindi rin masyadong flat.){/i}"

# game/logic/locations/corridor.rpy:1064
translate philippine train_not_too_big_e090f714:

    # ch_Name "{i}(The golden mean!){/i}"
    ch_Name  "{i}(Tamang-tama lang!){/i}"

# game/logic/locations/corridor.rpy:1066
translate philippine train_not_too_big_0a7859bf:

    # ch_Name "{i}(I wonder how it fits in my hand?){/i}"
    ch_Name "{i}(Paano kaya ito sa kamay ko?){/i}"

# game/logic/locations/corridor.rpy:1081
translate philippine train_not_too_big_c6590b10:

    # ch_Name "{i}(It's so soft to the touch!){/i}"
    ch_Name "{i}(Napakalambot kapag hawakan!){/i}"

# game/logic/locations/corridor.rpy:1085
translate philippine train_not_too_big_2b8ccd71:

    # ch_Name "{i}(And so warm.){/i}"
    ch_Name "{i}(At maligamgam.){/i}"

# game/logic/locations/corridor.rpy:1089
translate philippine train_not_too_big_768d59cf:

    # ch_Name "{i}(I must find out how firm it is!){/i}"
    ch_Name "{i}(Dapat kong malaman kung gaano kalambot ito!){/i}"

# game/logic/locations/corridor.rpy:1106
translate philippine train_not_too_big_625b1304:

    # ch_Name "{i}(Oh, yes! Divine.){/i}"
    ch_Name "{i}(Ay, oo! Banal.){/i}"

# game/logic/locations/corridor.rpy:1110
translate philippine train_not_too_big_1c4eae0a:

    # ch_Name "{i}(Even through clothes, I can feel how firm they are...){/i}"
    ch_Name "{i}(Kahit na may damit, ramdam ko kung gaano sila kaganda...){/i}"

# game/logic/locations/corridor.rpy:1114
translate philippine train_not_too_big_c33859f4:

    # ch_Name "{i}(Is it my imagination, or did Haley just moved?){/i}"
    ch_Name "{i}(Imagination ko ba ito, o gumalaw si Haley?){/i}"

# game/logic/locations/corridor.rpy:1118
translate philippine train_not_too_big_9d5e7730:

    # ch_Name "{i}(Better not take any chances.){/i}"
    ch_Name "{i}(Mas mabuting huwag makipagsapalaran.){/i}"

# game/logic/locations/corridor.rpy:1122
translate philippine train_not_too_big_eff1c28d:

    # ch_Name "{i}(Hmm.){/i}"
    ch_Name ""

# game/logic/locations/corridor.rpy:1126
translate philippine train_not_too_big_90ebcc12:

    # ch_Name "{i}(I think she's just tossing and turning in her sleep.){/i}"
    ch_Name "{i}(Sa tingin ko ay natutulog pa rin siya.){/i}"

# game/logic/locations/corridor.rpy:1130
translate philippine train_not_too_big_811c5148:

    # ch_Name "{i}(Even though I'm afraid to wake you up, Haley...){/i}"
    ch_Name "{i}(Kahit natatakot akong gisingin ka, Haley...){/i}"

# game/logic/locations/corridor.rpy:1134
translate philippine train_not_too_big_94c3c904:

    # ch_Name "{i}(I don't know if I can resist looking at your nipple...){/i}"
    ch_Name "{i}(Hindi ko alam kung kaya kong hindi tumingin sa utong mo...){/i}"

# game/logic/locations/corridor.rpy:1151
translate philippine train_not_too_big_deb23e4b:

    # ch_Name "{i}(Gently pinch the edge of her blouse with a finger...){/i}"
    ch_Name "{i}(Dahang kurutin ang gilid ng kanyang damit...){/i}"

# game/logic/locations/corridor.rpy:1155
translate philippine train_not_too_big_24c40400:

    # ch_Name "{i}(And... Voila!){/i}"
    ch_Name "{i}(At... Voila!){/i}"

# game/logic/locations/corridor.rpy:1159
translate philippine train_not_too_big_1bddf9ae:

    # ch_Name "{i}(Hello, hello!){/i}"
    ch_Name ""

# game/logic/locations/corridor.rpy:1163
translate philippine train_not_too_big_afeadfb4:

    # ch_Name "{i}(It's so neat.){/i}"
    ch_Name "{i}(Napakaayos nito.){/i}"

# game/logic/locations/corridor.rpy:1167
translate philippine train_not_too_big_252c8e49:

    # ch_Name "{i}(Haley, you're really beautiful...){/i}"
    ch_Name "{i}(Haley, ang ganda mo talaga...){/i}"

# game/logic/locations/corridor.rpy:1171
translate philippine train_not_too_big_69660076:

    # ch_Name "{i}(Huh?){/i}"
    ch_Name ""

# game/logic/locations/corridor.rpy:1175
translate philippine train_not_too_big_7c636307:

    # ch_Name "{i}(Turned away from me.){/i}"
    ch_Name "{i}(Tumalikod sa akin.){/i}"

# game/logic/locations/corridor.rpy:1179
translate philippine train_not_too_big_2352b182:

    # ch_Name "{i}(Probably tired of our \"games\".){/i}"
    ch_Name "{i}(Marahil ay pagod na sa ating \"mga laro\".){/i}"

# game/logic/locations/corridor.rpy:1183
translate philippine train_not_too_big_b5be0bab:

    # ch_Name "{i}(I'm not gonna push my luck anymore.){/i}"
    ch_Name "{i}(Hindi ko na ipipilit ang swerte ko.){/i}"

# game/logic/locations/corridor.rpy:1187
translate philippine train_not_too_big_a4963e7c:

    # ch_Name "{i}(You're done for the day.){/i}"
    ch_Name "{i}(Tapos ka na para sa araw na ito.){/i}"

# game/logic/locations/corridor.rpy:1191
translate philippine train_not_too_big_a0b38d9b:

    # ch_Name "{i}(But we'll get back to it!){/i}"
    ch_Name "{i}(Ngunit babalikan natin ito!){/i}"

# game/logic/locations/corridor.rpy:1195
translate philippine train_not_too_big_842b7743:

    # ch_Name "{i}(Won't we?){/i}"
    ch_Name "{i}(DIba?){/i}"

# game/logic/locations/corridor.rpy:1199
translate philippine train_not_too_big_7e8269c9:

    # ch_Name "{i}(What a rush of emotion...){/i}"
    ch_Name "{i}(Napakabilis ng mga emosyon...){/i}"

# game/logic/locations/corridor.rpy:1201
translate philippine train_not_too_big_261de453:

    # ch_Name "{i}(My heart's beating like crazy.){/i}"
    ch_Name "{i}(Parang baliw ang tibok ng puso ko.){/i}"

# game/logic/locations/corridor.rpy:1210
translate philippine train_end_16_1_ef9e3e55:

    # ch_Name "{i}(I'd better turn away from temptation...){/i}"
    ch_Name "{i}(Mas mabuting talikuran ko ang tukso...){/i}"

# game/logic/locations/corridor.rpy:1213
translate philippine train_end_16_1_e3c5e027:

    # ch_Name "{i}(...and try to sleep...){/i}"
    ch_Name "{i}(...at subukang matulog...){/i}"

# game/logic/locations/corridor.rpy:1220
translate philippine train_end_16_1_a0dc44c3:

    # Haley "Hey, [Name], wake up!"
    Haley "Hoy, [Name], gumising ka!"

# game/logic/locations/corridor.rpy:1222
translate philippine train_end_16_1_0089bfed:

    # ch_Name "Well, just a few more minutes..."
    ch_Name "Ilang minuto pa..."

# game/logic/locations/corridor.rpy:1224
translate philippine train_end_16_1_89382e47:

    # Haley "{i}(Hmm, he's cute when he sleeps.{/i})"
    Haley "{i}(Hmm, ang cute niya kapag natutulog.{/i})"

# game/logic/locations/corridor.rpy:1230
translate philippine train_end_16_1_04785988:

    # ch_Name "Did you say something?"
    ch_Name "May sinabi ka ba?"

# game/logic/locations/corridor.rpy:1234
translate philippine train_end_16_1_5ca9338c:

    # Haley "I said WAKE UP! You won't believe your eyes!"
    Haley "Sabi ko \"gumising ka\"! Hindi ka maniniwala sa makikita mo!"

# game/logic/locations/corridor.rpy:1238
translate philippine train_end_16_1_d3523126:

    # ch_Name "{i}(I'm sure she just called me cute...){/i}"
    ch_Name "{i}(Sigurado akong tinawag niya akong cute...){/i}"

# game/logic/locations/corridor.rpy:1242
translate philippine train_end_16_1_2c1e9e70:

    # ch_Name "{i}(Was that what she was thinking?){/i}"
    ch_Name "{i}(Iyon ba ang iniisip niya?){/i}"

# game/logic/locations/corridor.rpy:1246
translate philippine train_end_16_1_977eeda5:

    # ch_Name "Haley, what happened?"
    ch_Name "Haley, ano ang nangyari?"

# game/logic/locations/corridor.rpy:1250
translate philippine train_end_16_1_e0a0110f:

    # ch_Name "I was sleeping so sweet..."
    ch_Name "Ang sarap ng tulog ko..."

# game/logic/locations/corridor.rpy:1254
translate philippine train_end_16_1_453de76b:

    # Haley "Tell me you're seeing this, too. I'm not hallucinating, am I?"
    Haley "Sabihin mo nakikita mo rin ito. Hindi naman ako baliw diba?"

# game/logic/locations/corridor.rpy:1258
translate philippine train_end_16_1_e4d001fe:

    # ch_Name "Holy crap!"
    ch_Name "Diyos ko!"

# game/logic/locations/corridor.rpy:1262
translate philippine train_end_16_1_170850cd:

    # ch_Name "Am I still dreaming?"
    ch_Name "Nanaginip pa ba ako?"

# game/logic/locations/corridor.rpy:1267
translate philippine train_end_16_1_c212acb2:

    # ch_Name "Fucking dragons!"
    ch_Name "Mga dragon!"

# game/logic/locations/corridor.rpy:1271
translate philippine train_end_16_1_3e74e358:

    # ch_Name "Unbelievable..."
    ch_Name "Hindi kapani-paniwala..."

# game/logic/locations/corridor.rpy:1275
translate philippine train_end_16_1_f1e164fe:

    # Haley "Magical..."
    Haley "Mahiwagang..."

# game/logic/locations/corridor.rpy:1280
translate philippine train_end_16_1_346beea5:

    # Haley "I've only seen these in movies."
    Haley "Nakikita ko lang ito sa mga pelikula."

# game/logic/locations/corridor.rpy:1284
translate philippine train_end_16_1_a6679a5b:

    # ch_Name "Yeah..."
    ch_Name "Oo..."

# game/logic/locations/corridor.rpy:1288
translate philippine train_end_16_1_68896626:

    # Haley "Do you think they can attack a train?"
    Haley "Sa tingin mo kaya nilang atakihin ang isang tren?"

# game/logic/locations/corridor.rpy:1293
translate philippine train_end_16_1_fb1f3628:

    # ch_Name "Those cuties?"
    ch_Name "Sila?"

# game/logic/locations/corridor.rpy:1298
translate philippine train_end_16_1_c230c6ce:

    # ch_Name "I don't think so!"
    ch_Name "Siguro hindi!"

# game/logic/locations/corridor.rpy:1302
translate philippine train_end_16_1_d0af53fc:

    # ch_Name "They seem pretty peaceful."
    ch_Name "Mukhang mapayapa sila."

# game/logic/locations/corridor.rpy:1307
translate philippine train_end_16_1_d61fc905:

    # Haley "I don't know, it's kind of creepy."
    Haley "Hindi ko alam, medyo nakakatakot."

# game/logic/locations/corridor.rpy:1311
translate philippine train_end_16_1_8ef81719:

    # Haley "I thought all dragons were predators!"
    Haley "Akala ko lahat ng dragon ay mandaragit!"

# game/logic/locations/corridor.rpy:1315
translate philippine train_end_16_1_ae8734d1:

    # ch_Name "Haley, don't worry about it."
    ch_Name "Haley, huwag kang mag-alala."

# game/logic/locations/corridor.rpy:1319
translate philippine train_end_16_1_17fa1d94:

    # ch_Name "They would have attacked a long time ago!"
    ch_Name "Matagal na sana silang umatake!"

# game/logic/locations/corridor.rpy:1323
translate philippine train_end_16_1_d3c17d2c:

    # Haley "I guess you're right."
    Haley "Siguro tama ka."

# game/logic/locations/corridor.rpy:1327
translate philippine train_end_16_1_c488fc38:

    # Haley "Sorry, I'm a little afraid of all lizards..."
    Haley "Paumanhin, medyo natatakot ako sa mga butiki..."

# game/logic/locations/corridor.rpy:1331
translate philippine train_end_16_1_eb8c6edb:

    # Haley "Especially flying ones."
    Haley "Lalo na ang mga lumilipad."

# game/logic/locations/corridor.rpy:1335
translate philippine train_end_16_1_6a410dd4:

    # ch_Name "I wonder what other surprises this day has in store for us."
    ch_Name "Nagtataka ako kung ano ang iba pang mga sorpresa sa araw na ito para sa atin."

# game/logic/locations/corridor.rpy:1339
translate philippine train_end_16_1_2fc24f18:

    # Haley "I guess we're about to find out!"
    Haley "Malalaman na kaya natin!"

# game/logic/locations/corridor.rpy:1344
translate philippine test_label_ep1_1_63bf38a9:

    # Haley "There's a tunnel ahead."
    Haley "May lagusan na doon."

# game/logic/locations/corridor.rpy:1354
translate philippine test_label_ep1_1_aa81c689:

    # ch_Name "...What a long tunnel..."
    ch_Name "...Ang haba ng lagusan..."

# game/logic/locations/corridor.rpy:1356
translate philippine test_label_ep1_1_dba360aa:

    # Haley "It's like an eternity."
    Haley "Parang walang hanggan."

# game/logic/locations/corridor.rpy:1358
translate philippine test_label_ep1_1_919dda1c:

    # Haley "In some cultures, a dark tunnel symbolizes the passage between worlds."
    Haley "Sa ibang kultura, ang madilim na lagusan ay sumisimbolo sa daanan sa pagitan ng mga mundo."

# game/logic/locations/corridor.rpy:1360
translate philippine test_label_ep1_1_ff4bddfb:

    # Haley "Or the afterlife..."
    Haley "O ang kabilang buhay..."

# game/logic/locations/corridor.rpy:1362
translate philippine test_label_ep1_1_1287c9b0:

    # ch_Name "How do you know all this?"
    ch_Name "Paano mo nalaman ang lahat ng ito?"

# game/logic/locations/corridor.rpy:1364
translate philippine test_label_ep1_1_4100aa13:

    # Haley "My mother's a librarian."
    Haley "Librarian ang nanay ko."

# game/logic/locations/corridor.rpy:1368
translate philippine test_label_ep1_1_adfef7d0:

    # Haley "Look, I think we're almost there!"
    Haley "Tingnan mo, malapit na yata tayo!"

# game/logic/locations/corridor.rpy:1370
translate philippine test_label_ep1_1_e63181c2:

    # ch_Name "Yeah, I think dragons were a strong hint."
    ch_Name "Oo, ko malakas na pahiwatig ang mga dragon."

# game/logic/locations/corridor.rpy:1378
translate philippine test_label_ep1_1_ffd0dea4:

    # Haley "You never know..."
    Haley "Hindi mo malalaman..."

# game/logic/locations/corridor.rpy:1382
translate philippine test_label_ep1_1_fac1f63a:

    # "Radio" "Applicants for the Cordale entrance exam..."
    "Radio" "Mga aplikante para sa Cordale Entrance Exam..."

# game/logic/locations/corridor.rpy:1386
translate philippine test_label_ep1_1_3d481eee:

    # "Radio" "...please gather in the lobby of the station within 10 minutes. Don't be late."
    "Radio" "...magtipon sa istasyon sa loob ng 10 minuto. Huwag magpahuli."

# game/logic/locations/corridor.rpy:1390
translate philippine test_label_ep1_1_f01eb552:

    # Haley "So, into the unknown?"
    Haley "Kaya, tara na?"

# game/logic/locations/corridor.rpy:1394
translate philippine test_label_ep1_1_6267bfe7:

    # Haley "[Name], hey, you coming?"
    Haley "[Name], sasama ka?"

# game/logic/locations/corridor.rpy:1398
translate philippine test_label_ep1_1_654929b0:

    # ch_Name "Go without me."
    ch_Name "Mag-uuna ka na."

# game/logic/locations/corridor.rpy:1402
translate philippine test_label_ep1_1_8df2eae4:

    # ch_Name "I have to meet a friend."
    ch_Name "Kailangan kong hanapin ang aking kaibigan."

# game/logic/locations/corridor.rpy:1406
translate philippine test_label_ep1_1_7d9168ea:

    # ch_Name "I'll find you later."
    ch_Name "Hahanapin kita mamaya."

# game/logic/locations/corridor.rpy:1414
translate philippine test_label_ep1_1_354a9cc9:

    # ch_Name "{i}(I need to find Samantha. Where was the meeting point?){/i}"
    ch_Name "{i}(Kailangan kong hanapin si Samantha. Saan ang tagpuan?){/i}"

# game/logic/locations/corridor.rpy:1418
translate philippine test_label_ep1_1_939c46f3:

    # ch_Name "{i}(Ah! Think of the devil and here is!){/i}"
    ch_Name "{i}(Ah! Isipin mo ang diyablo at magpapakita siya!){/i}"

# game/logic/locations/corridor.rpy:1422
translate philippine test_label_ep1_1_add9410c:

    # Samantha "[Name], there you are!"
    Samantha "[Name], nariyan ka na!"

# game/logic/locations/corridor.rpy:1426
translate philippine test_label_ep1_1_184a2fb8:

    # Samantha "Let's step aside."
    Samantha "Tabi tayo."

# game/logic/locations/corridor.rpy:1430
translate philippine test_label_ep1_1_e46aa815:

    # ch_Name "Samantha! I was just looking for you!"
    ch_Name "Samantha! Hinanap nga kita!"

# game/logic/locations/corridor.rpy:1434
translate philippine test_label_ep1_1_ca60e23b:

    # Samantha "Not here."
    Samantha "Hindi dito."

# game/logic/locations/corridor.rpy:1438
translate philippine test_label_ep1_1_b6d3dd39:

    # Samantha "Follow me."
    Samantha "Sundin mo ako."

# game/logic/locations/corridor.rpy:1444
translate philippine test_label_ep1_1_981dad95:

    # ch_Name "Sam, wait up! Where are we going?"
    ch_Name "Sam, teka! Saan tayo pupunta?"

# game/logic/locations/corridor.rpy:1448
translate philippine test_label_ep1_1_76c7b076:

    # Samantha "Shh! Speak quietly. "
    Samantha "Shh! Magsalita ng tahimik."

# game/logic/locations/corridor.rpy:1452
translate philippine test_label_ep1_1_0c755b8e:

    # Samantha "I came to warn you."
    Samantha "Naparito ako para balaan ka."

# game/logic/locations/corridor.rpy:1456
translate philippine test_label_ep1_1_90c322f2:

    # Samantha "As I've promised I will."
    Samantha "Sabi ko naman, diba."

# game/logic/locations/corridor.rpy:1460
translate philippine test_label_ep1_1_0e717334:

    # ch_Name "About what?"
    ch_Name "Tungkol sa?"

# game/logic/locations/corridor.rpy:1464
translate philippine test_label_ep1_1_9e619138:

    # Samantha "About the entrance exam. "
    Samantha "Tungkol sa pagsubok."

# game/logic/locations/corridor.rpy:1468
translate philippine test_label_ep1_1_4ffc325c:

    # Samantha "Did you notice anything unusual while on the train?"
    Samantha "May napansin ka bang kakaiba habang nasa tren?"

# game/logic/locations/corridor.rpy:1472
translate philippine test_label_ep1_1_9d29fade:

    # ch_Name "You mean like dragons outside the window?"
    ch_Name "Tulad ng mga dragon sa labas ng bintana?"

# game/logic/locations/corridor.rpy:1476
translate philippine test_label_ep1_1_8fd00b75:

    # ch_Name "Of course I noticed dragons, they are freaking dragons, come on!"
    ch_Name "Syempre napansin ko ang mga dragon!"

# game/logic/locations/corridor.rpy:1480
translate philippine test_label_ep1_1_b39c85d6:

    # Samantha "I'm not talking about that, you fool. "
    Samantha "Hindi ko sinasabi iyan, tanga."

# game/logic/locations/corridor.rpy:1484
translate philippine test_label_ep1_1_1db51077:

    # Samantha "Did you notice anything unusual behind you?"
    Samantha "May napansin ka bang kakaiba?"

# game/logic/locations/corridor.rpy:1488
translate philippine test_label_ep1_1_91e785f7:

    # Samantha "Did you feel... powerful?"
    Samantha "Naramdaman mo bang... makapangyarihan ka?"

# game/logic/locations/corridor.rpy:1492
translate philippine test_label_ep1_1_066bb089:

    # ch_Name "{i}(I thought I could hear Haley's thoughts.){/i}"
    ch_Name {i}(Akala ko naririnig ko ang iniisip ni Haley.){/i}"

# game/logic/locations/corridor.rpy:1496
translate philippine test_label_ep1_1_e382213f:

    # ch_Name "{i}(Should I tell Sam about this?){/i}"
    ch_Name "{i}(Dapat ko bang sabihin kay Sam ang tungkol dito?){/i}"

# game/logic/locations/corridor.rpy:1508
translate philippine test_label_ep1_1_e6fe81a2:

    # Samantha "Is that true? What am I thinking right now?"
    Samantha "Totoo ba yun? Ano ba ang iniisip ko ngayon?"

# game/logic/locations/corridor.rpy:1514
translate philippine end_no_nothing_855666ac:

    # Samantha "Don't be afraid of me, pal. I am trying to help."
    Samantha Huwag kang matakot sa akin. Sinusubukan ko lang tumulong sa iyo."

# game/logic/locations/corridor.rpy:1516
translate philippine end_no_nothing_f7b247cf:

    # ch_Name "I'm not even sure anything happened."
    ch_Name "Hindi rin ako sigurado na may nangyari."

# game/logic/locations/corridor.rpy:1518
translate philippine end_no_nothing_edbc7c5d:

    # Samantha "What could have happened?"
    Samantha "Ano kayang nangyari?"

# game/logic/locations/corridor.rpy:1520
translate philippine end_no_nothing_191158e4:

    # Samantha "Sparks from your fingers?"
    Samantha "Kidlat mula sa iyong mga daliri?"

# game/logic/locations/corridor.rpy:1522
translate philippine end_no_nothing_a7ca91d9:

    # Samantha "Did you shatter glass just by looking at it?"
    Samantha "Nabasag mo ba ang salamin sa pagtingin?"

# game/logic/locations/corridor.rpy:1524
translate philippine end_no_nothing_df06b3a5:

    # Samantha "Did you read someone's thoughts?"
    Samantha "Nabasa mo ba ang iniisip ng isang tao?"

# game/logic/locations/corridor.rpy:1526
translate philippine end_no_nothing_902f5322:

    # ch_Name "How did you know?"
    ch_Name "Paano mo nalaman?"

# game/logic/locations/corridor.rpy:1528
translate philippine end_no_nothing_31ab1b88:

    # Samantha "I know everything! Remember that. Come on, read my mind!"
    Samantha "Alam ko ang lahat! Tandaan mo 'yan. Halika, basahin mo ang isip ko!"

# game/logic/locations/corridor.rpy:1530
translate philippine end_no_nothing_537e3051:

    # ch_Name "I don't know how to do it on purpose..."
    ch_Name "Hindi ko alam kung paano ito gagawin ng kusa..."

# game/logic/locations/corridor.rpy:1532
translate philippine end_no_nothing_177dee75:

    # ch_Name "I'm not at all sure that that happened."
    ch_Name "Hindi ako sigurado kung paano nangyari iyon."

# game/logic/locations/corridor.rpy:1534
translate philippine end_no_nothing_5239c40e:

    # Samantha "Make sure you remember one thing: in this place nothing \"seems\" a certain way without reason."
    Samantha "Tandaan mo: sa lugar na ito walang \"tila\" sa isang tiyak na paraan nang walang dahilan."

# game/logic/locations/corridor.rpy:1536
translate philippine end_no_nothing_b57e43d9:

    # Samantha "Concentrate and look me in the eyes."
    Samantha "Magconcentrate at tumingin sa mga mata ko. "

# game/logic/locations/corridor.rpy:1544
translate philippine Read_Samanthas_thoughts_label_75fed526:

    # "{i}Sam is humming Avril Lavigne - Girlfriend in her mind.{/i}"
    "{i}Kumakanta si Sam mga Avril Lavigne - Girlfriend sa isip niya.{/i}"

# game/logic/locations/corridor.rpy:1546
translate philippine Read_Samanthas_thoughts_label_90ddfdc6:

    # ch_Name "You have terrible taste in music."
    ch_Name "Ang pangit ng mga musikang gusto mo."

# game/logic/locations/corridor.rpy:1548
translate philippine Read_Samanthas_thoughts_label_35f374ae:

    # Samantha "Very cool! Try again!"
    Samantha "Astig! Subukan ulit!"

# game/logic/locations/corridor.rpy:1554
translate philippine Read_Samanthas_thoughts_label_b592d5b3:

    # "{i}Samantha's thoughts: (Inaudible interference.){/i}"
    "{i}Ang mga iniisip ni Samantha: (Hindi marinig na panghihimasok.){/i}"

# game/logic/locations/corridor.rpy:1556
translate philippine Read_Samanthas_thoughts_label_acdef95d:

    # ch_Name "I... I can't..."
    ch_Name "I... hindi ko kaya..."

# game/logic/locations/corridor.rpy:1558
translate philippine Read_Samanthas_thoughts_label_861e54b9:

    # Samantha "This is called a \"mental block.\""
    Samantha "Tinatawag itong \"mental block.\""

# game/logic/locations/corridor.rpy:1560
translate philippine Read_Samanthas_thoughts_label_89231c2e:

    # Samantha "A telepath of your level will not be able to read the thoughts of a talented magician like me."
    Samantha "Ang isang telepath tulad mo ay hindi makakabasa ng iniisip ng isang mahuhusay na salamangkero tulad ko."

# game/logic/locations/corridor.rpy:1562
translate philippine Read_Samanthas_thoughts_label_bd9158ad:

    # Samantha "The first time I “let” you do it."
    Samantha "Sa unang pagkakataon \"hinayaan\" kitang gawin ito."

# game/logic/locations/corridor.rpy:1564
translate philippine Read_Samanthas_thoughts_label_5a8cbd85:

    # ch_Name "Sounds like a useless ability."
    ch_Name "Parang walang kwentang kakayahan."

# game/logic/locations/corridor.rpy:1566
translate philippine Read_Samanthas_thoughts_label_89d75694:

    # Samantha "You will have plenty of time to practice your skill."
    Samantha "Magkakaroon ka ng maraming oras para sanayin ang iyong kakayahan."

# game/logic/locations/corridor.rpy:1568
translate philippine Read_Samanthas_thoughts_label_3ef9f68f:

    # Samantha "If you don't fail the entrance exams..."
    Samantha "Kung hindi ka bumagsak sa pagsubok..."

# game/logic/locations/corridor.rpy:1570
translate philippine Read_Samanthas_thoughts_label_ccfb137f:

    # Samantha "...and your memory isn't erased."
    Samantha "...at hindi nabubura ang iyong alaala."

# game/logic/locations/corridor.rpy:1572
translate philippine Read_Samanthas_thoughts_label_bacd6e13:

    # ch_Name "My memory isn't erased?!"
    ch_Name "Hindi nabubura ang alaala ko?!"

# game/logic/locations/corridor.rpy:1574
translate philippine Read_Samanthas_thoughts_label_5aa7f8a1:

    # Samantha "Damn, that's why I was looking for you."
    Samantha "Bwisit, kaya pala hinanap kita."

# game/logic/locations/corridor.rpy:1576
translate philippine Read_Samanthas_thoughts_label_e8dcf684:

    # Samantha "Listen!"
    Samantha "Makinig ka!"

# game/logic/locations/corridor.rpy:1578
translate philippine Read_Samanthas_thoughts_label_f75a62da:

    # Samantha "All freshmen will have an entrance exam in magic. "
    Samantha "Lahat ng freshmen ay magkakaroon ng pagsubok sa mahika."

# game/logic/locations/corridor.rpy:1580
translate philippine Read_Samanthas_thoughts_label_bb4487e3:

    # Samantha "If you fail, your memory will be erased and you'll be sent back to the world without magic."
    Samantha "Kung mabigo ka, mabubura ang iyong alaala at ibabalik ka sa mundo nang walang mahika."

# game/logic/locations/corridor.rpy:1582
translate philippine Read_Samanthas_thoughts_label_31a35028:

    # Samantha "And you will never be able to come here again. Ever."
    Samantha "At hindi ka na makakabalik dito. Kailanman."

# game/logic/locations/corridor.rpy:1584
translate philippine Read_Samanthas_thoughts_label_5f55f2f9:

    # ch_Name "What should I do?"
    ch_Name "Ano ba ang gagawin ko?"

# game/logic/locations/corridor.rpy:1586
translate philippine Read_Samanthas_thoughts_label_94837700:

    # Samantha "Don't fail the exam."
    Samantha "Huwag kang bumagsak sa pagsusulit."

# game/logic/locations/corridor.rpy:1588
translate philippine Read_Samanthas_thoughts_label_26c73589:

    # ch_Name "And this is your wise advice?!"
    ch_Name "At ito ang iyong matalinong payo?!"

# game/logic/locations/corridor.rpy:1590
translate philippine Read_Samanthas_thoughts_label_95264c4f:

    # Samantha "Let me finish, you goof!"
    Samantha "Hayaan mo akong tapusin, ang tanga mo!"

# game/logic/locations/corridor.rpy:1592
translate philippine Read_Samanthas_thoughts_label_85f1d0f3:

    # Samantha "Magic is powered by Passion. Remember this during the exam."
    Samantha "Ang Mahika ang gumagana dahil sa Hilig. Tandaan mo ito."

# game/logic/locations/corridor.rpy:1594
translate philippine Read_Samanthas_thoughts_label_2e16d0e1:

    # Samantha "Focus on something that you have a passion for and you will definitely succeed."
    Samantha "Mag-isip ka sa bagay na hilig mo at siguradong magtatagumpay ka."

# game/logic/locations/corridor.rpy:1596
translate philippine Read_Samanthas_thoughts_label_ad676582:

    # ch_Name "But why am I just now... starting to read minds?"
    ch_Name "Pero bakit ngayon lang ako... nagsimulang magbasa ng isip?"

# game/logic/locations/corridor.rpy:1598
translate philippine Read_Samanthas_thoughts_label_5d204ef4:

    # Samantha "This is no ordinary train. "
    Samantha "Ito ay hindi ordinaryong tren."

# game/logic/locations/corridor.rpy:1600
translate philippine Read_Samanthas_thoughts_label_fe1e6bdf:

    # Samantha "To prepare students for the difficult exam..."
    Samantha "Para ihanda ang mga estudyante sa mahirap na pagsusulit..."

# game/logic/locations/corridor.rpy:1602
translate philippine Read_Samanthas_thoughts_label_3a5969fd:

    # Samantha "...the Cordale train saturates everything inside with magical power."
    Samantha "...ang tren ng Cordale ay nagbababad nga mahika sa lahat sa loob nito."

# game/logic/locations/corridor.rpy:1604
translate philippine Read_Samanthas_thoughts_label_75c459dd:

    # Samantha "While you slept, it charged you with power, like a battery."
    Samantha "Habang natutulog ka, binigyan ka nito ng kapangyarihan, tulad ng isang baterya."

# game/logic/locations/corridor.rpy:1606
translate philippine Read_Samanthas_thoughts_label_f594ccec:

    # Radio "First year students..."
    Radio "Mga mag-aaral sa unang taon..."

# game/logic/locations/corridor.rpy:1608
translate philippine Read_Samanthas_thoughts_label_764a091a:

    # Radio "...who do not appear on the platform within a minute..."
    Radio "...na hindi lumilitaw sa entablado sa loob ng isang minuto..."

# game/logic/locations/corridor.rpy:1610
translate philippine Read_Samanthas_thoughts_label_490283c6:

    # Radio "...will lose the opportunity to take part in the entrance exam."
    Radio "... aymawawalan ng pagkakataon na sumali sa pagsubok."

# game/logic/locations/corridor.rpy:1612
translate philippine Read_Samanthas_thoughts_label_57489638:

    # ch_Name "Damn, I gotta run! Thanks for the advice!"
    ch_Name "Naku, kailangan ko nang umalis! Salamat sa payo!"

# game/logic/locations/corridor.rpy:1614
translate philippine Read_Samanthas_thoughts_label_9fa400f5:

    # Samantha "Wait."
    Samantha "Teka."

# game/logic/locations/corridor.rpy:1616
translate philippine Read_Samanthas_thoughts_label_abc10325:

    # Samantha "Good luck!"
    Samantha ""

# game/logic/locations/corridor.rpy:1618
translate philippine Read_Samanthas_thoughts_label_5f912d8f:

    # ch_Name "{i}(This is exactly what you can be passionate about.){/i}"
    ch_Name "{i}(Ito mismo ang Hilig ko.){/i}"

translate philippine strings:

    # game/logic/locations/corridor.rpy:45
    old "Jackpot"
    new ""

    # game/logic/locations/corridor.rpy:293
    old "Go to your room"
    new "Pumunta sa silid mo"

    # game/logic/locations/corridor.rpy:309
    old "Continue watching"
    new "Ituloy ang panonood"

    # game/logic/locations/corridor.rpy:736
    old "Offer assistance"
    new "Mag-alok ng tulong"

    # game/logic/locations/corridor.rpy:736
    old "Enjoy the view"
    new "Magtamasa ng tanawin"

    # game/logic/locations/corridor.rpy:763
    old "Continue enjoying the view"
    new "Magpatuloy sa pagtamasa ng tanawin"

    # game/logic/locations/corridor.rpy:815
    old "Of course not!"
    new "Siyempre hindi!"

    # game/logic/locations/corridor.rpy:815
    old "Two drunk girls!?"
    new "Dalawang lasing na babae!?"

    # game/logic/locations/corridor.rpy:940
    old "Take a look at Haley"
    new "Tingnan si Haley"

    # game/logic/locations/corridor.rpy:953
    old "Look under her skirt"
    new "Tingnan ang ilalim ng palda niya"

    # game/logic/locations/corridor.rpy:953
    old "Turn her on her back"
    new "Patalikod siya"

    # game/logic/locations/corridor.rpy:953
    old "Stop & go to sleep"
    new "Tumigil at matulog na"

    # game/logic/locations/corridor.rpy:986
    old "Touch"
    new "Hawakan"

    # game/logic/locations/corridor.rpy:1013
    old "Move her panties"
    new "Ilipat ang kanyang panloob"

    # game/logic/locations/corridor.rpy:1068
    old "Grab it"
    new "Yakapin"

    # game/logic/locations/corridor.rpy:1093
    old "Rub her breasts"
    new "Kuskusin ang kanyang dibdib"

    # game/logic/locations/corridor.rpy:1138
    old "Bare nipple"
    new "Hubad na utong"

    # game/logic/locations/corridor.rpy:1500
    old "I heard thoughts"
    new "Narinig ko ang mga iniisip"

    # game/logic/locations/corridor.rpy:1500
    old "No, nothing"
    new "Hindi, wala"

    # game/logic/locations/corridor.rpy:1540
    old "Read Samantha's thoughts"
    new "Basahin ang iniisip ni Samantha"

