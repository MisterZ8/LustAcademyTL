# TODO: Translation updated at 2022-01-30 19:04

# game/logic/events/giant.rpy:27
translate philippine giant_1_label_e6b11dbc:

    # ch_Name "{i}(Well, that was interesting.){/i}"
    ch_Name "{i}(Nakakaiba naman.){/i}"

# game/logic/events/giant.rpy:28
translate philippine giant_1_label_af0e151e:

    # ch_Name "{i}(But exhausting.){/i}"
    ch_Name "{i}(Pero napagod na ako.){/i}"

# game/logic/events/giant.rpy:29
translate philippine giant_1_label_524e5b0a:

    # ch_Name "{i}(I'd better go get some sleep.){/i}"
    ch_Name "{i}(Matulog na siguro ako.){/i}"

# game/logic/events/giant.rpy:33
translate philippine giant_1_label_90b439bd:

    # ch_Name "{i}(It's getting really late and the club alley is only getting more crowded.){/i}"
    ch_Name "{i}(Gabi na at punong-puno na ang eskenita.){/i}"

# game/logic/events/giant.rpy:34
translate philippine giant_1_label_5f763497:

    # ch_Name "{i}(Do people here even sleep?){/i}"
    ch_Name "{i}(Natutulog ba ang mga tao dito?){/i}"

# game/logic/events/giant.rpy:38
translate philippine giant_1_label_59c4b5fd:

    # ch_Name "{i}(Oh, main street, on the other hand, is almost empty.){/i}"
    ch_Name "{i}(Walang tao sa kalye.){/i}"

# game/logic/events/giant.rpy:39
translate philippine giant_1_label_8a1fc90f:

    # ch_Name "{i}(But who is that strange creature?){/i}"
    ch_Name "{i}(Sino ba 'yan?){/i}"

# game/logic/events/giant.rpy:40
translate philippine giant_1_label_781d9f29:

    # ch_Name "{i}(I'm almost sure he's watching my every step.){/i}"
    ch_Name "{i}(Nanonood siya sa mga ginagawa ko?){/i}"

# game/logic/events/giant.rpy:43
translate philippine giant_1_label_96867ce2:

    # ch_Name "{i}(The creature looks at me strangely.){/i}"
    ch_Name "{i}(Tinitignan niya talaga ako.){/i}"

# game/logic/events/giant.rpy:44
translate philippine giant_1_label_2051de2b:

    # ch_Name "{i}(It's bugging me.){/i}"
    ch_Name "{i}(Naiinis ako.){/i}"

# game/logic/events/giant.rpy:45
translate philippine giant_1_label_f525fa43:

    # ch_Name "{i}(I need to find out what it wants.){/i}"
    ch_Name "{i}(Gusto ko malaman ang gusto niya.){/i}"

# game/logic/events/giant.rpy:47
translate philippine giant_1_label_8676d769:

    # ch_Name "Hey..."
    ch_Name ""

# game/logic/events/giant.rpy:48
translate philippine giant_1_label_0931de10:

    # ch_Name "Hey... Man... You okay?"
    ch_Name "Hey... Ayos ka lang ba?"

# game/logic/events/giant.rpy:50
translate philippine giant_1_label_1932b048:

    # ch_Name "{i}(I don't like this. He seems to be reaching out to me...){/i}"
    ch_Name "{i}(Nakakaiba ito. Parang inaabot niya ako...){/i}"

# game/logic/events/giant.rpy:51
translate philippine giant_1_label_a63c50d4:

    # ch_Name "{i}(I'll probably regret this...){/i}"
    ch_Name "{i}(Sana hindi ito ang maling desisyon.){/i}"

# game/logic/events/giant.rpy:53
translate philippine giant_1_label_0c318f89:

    # ch_Name "{i}(What kind of creature is this... Terrifying sight...){/i}"
    ch_Name "{i}(Anong klase ng hayop ito... Nakakatakot...){/i}"

# game/logic/events/giant.rpy:54
translate philippine giant_1_label_bc556efa:

    # ch_Name "Hey. Is there anything I can do to help?"
    ch_Name "Hey. Makakatulong ba ako?"

# game/logic/events/giant.rpy:56
translate philippine giant_1_label_3b592f2c:

    # Giant "Argh..."
    Giant ""

# game/logic/events/giant.rpy:57
translate philippine giant_1_label_7a0e2657:

    # ch_Name "{i}(What the hell... Did it try to grab and eat me?){/i}"
    ch_Name "{i}(Ano ba ang... Kakainin niya ba ako?){/i}"

# game/logic/events/giant.rpy:59
translate philippine giant_1_label_de6a29d5:

    # ch_Name "I warn you, I'm a wizard!"
    ch_Name "Binabalaan kita, salamangkero ako!"

# game/logic/events/giant.rpy:60
translate philippine giant_1_label_c394a369:

    # Giant "Argh... N-no... Don't hurt me!"
    Giant "Argh... Hindi... 'Wag mo akong saktan!"

# game/logic/events/giant.rpy:62
translate philippine giant_1_label_13615c40:

    # Giant "Don't hurt!.. me..."
    Giant "'Wag! Ako!"

# game/logic/events/giant.rpy:63
translate philippine giant_1_label_c11f8925:

    # Giant "LOOK WHAT THEY DID TO ME..."
    Giant "TIGNAN MO ANG GINAWA NILA SA AKIN..."

# game/logic/events/giant.rpy:65
translate philippine giant_1_label_8eb6dc9d:

    # Giant "DO YOU SEE?!"
    Giant "MAKIKITA MO BA?!"

# game/logic/events/giant.rpy:66
translate philippine giant_1_label_dc8b5566:

    # ch_Name "{i}(What the hell is going on... What is he doing?){/i}"
    ch_Name "{i}(Ano ba ang nangyayari...Ano ang ginagawa niya?){/i}"

# game/logic/events/giant.rpy:68
translate philippine giant_1_label_5f829599:

    # ch_Name "{i}(He looks like a lunatic...){/i}"
    ch_Name "{i}(Parang baliw siya...){/i}"

# game/logic/events/giant.rpy:69
translate philippine giant_1_label_2255db69:

    # ch_Name "{i}(He also looks around to make sure we're alone...){/i}"
    ch_Name "{i}(Gusto niya na wala kaming kasama...){/i}"

# game/logic/events/giant.rpy:71
translate philippine giant_1_label_b4589463:

    # ch_Name "W... what do you want?!"
    ch_Name "A... Ano ba ang gusto mo?!"

# game/logic/events/giant.rpy:72
translate philippine giant_1_label_f34efcbc:

    # Giant "Argh... I'm confused."
    Giant "Argh... Nalilito ako."

# game/logic/events/giant.rpy:73
translate philippine giant_1_label_7799b362:

    # Giant "Look..."
    Giant "Makinig ka..."

# game/logic/events/giant.rpy:75
translate philippine giant_1_label_2fd601bc:

    # Giant "You're not one of them, are you?!"
    Giant "Kasali ka ba sa kanila?!"

# game/logic/events/giant.rpy:76
translate philippine giant_1_label_2e0430d5:

    # ch_Name "Who are you talking about?"
    ch_Name "Ano ang sinasabi mo?"

# game/logic/events/giant.rpy:77
translate philippine giant_1_label_d5354e3e:

    # ch_Name "What happened?!"
    ch_Name "Ano ang nangyari?!"

# game/logic/events/giant.rpy:78
translate philippine giant_1_label_71ccbe85:

    # Giant "Argh... I... will."
    Giant "Argh... Ako ay..."

# game/logic/events/giant.rpy:80
translate philippine giant_1_label_b80462a2:

    # Giant "But promise... help!"
    Giant "Mangako ka... tulungan mo ako!"

# game/logic/events/giant.rpy:88
translate philippine giant_1_label_41edf95c:

    # ch_Name "Sorry, I'm not going to help a creepy stranger."
    ch_Name "Patawad, pero 'di ako tutulong sa kakaibang estranghero."

# game/logic/events/giant.rpy:90
translate philippine giant_1_label_a9981ea3:

    # Giant "You're with them!"
    Giant "Isa ka sa kanila!"

# game/logic/events/giant.rpy:91
translate philippine giant_1_label_6bc07948:

    # Giant "You're one of them! YOU'RE ONE OF THEM!"
    Giant "Isa ka sa kanila! ISA KA SA KANILA!"

# game/logic/events/giant.rpy:93
translate philippine giant_1_label_4871c555:

    # Giant "WHAT HAVE YOU DONE TO ME!"
    Giant "ANO ANG GINAWA MO SA AKIN!"

# game/logic/events/giant.rpy:94
translate philippine giant_1_label_008d75e2:

    # ch_Name "{i}(What the hell!){/i}"
    ch_Name "{i}(Ano ba ito!){/i}"

# game/logic/events/giant.rpy:95
translate philippine giant_1_label_6ac6b442:

    # ch_Name "{i}(I think he's really pissed off.){/i}"
    ch_Name "{i}(Nagalit siya.){/i"

# game/logic/events/giant.rpy:97
translate philippine giant_1_label_bcb05f60:

    # Giant "Ah-argh!"
    Giant ""

# game/logic/events/giant.rpy:98
translate philippine giant_1_label_e40e7ea6:

    # ch_Name "That's it, I warned you!"
    ch_Name "Sabi ko sa iyo salamangkero ako!"

# game/logic/events/giant.rpy:100
translate philippine giant_1_label_33b7a34b:

    # ch_Name "Eat this!" with hpunch
    ch_Name "Heto!" with hpunch

# game/logic/events/giant.rpy:101
translate philippine giant_1_label_b9cfb0db:

    # Giant "Aaargh!"
    Giant ""

# game/logic/events/giant.rpy:103
translate philippine giant_1_label_4add1191:

    # Giant "What have you done to me!!!"
    Giant "Ano ang ginawa mo sa akin!!!"

# game/logic/events/giant.rpy:104
translate philippine giant_1_label_ef534a43:

    # Giant "Argh! YOU!"
    Giant "Argh! IKAW!"

# game/logic/events/giant.rpy:106
translate philippine giant_1_label_963fd335:

    # Giant "Leave me alone!"
    Giant "Iwanan mo ako!"

# game/logic/events/giant.rpy:107
translate philippine giant_1_label_fd29a07b:

    # Giant "Argh!!!"
    Giant ""

# game/logic/events/giant.rpy:109
translate philippine giant_1_label_22c8ad77:

    # ch_Name "{i}(He doesn't seem to be chasing me...){/i}"
    ch_Name "{i}(Hindi niya ako hinahabol...){/i}"

# game/logic/events/giant.rpy:110
translate philippine giant_1_label_a2bc7378:

    # ch_Name "{i}(But It's better not to wander the streets like this at night, by myself.){/i}"
    ch_Name "{i}(Pero hindi talaga ligtas ang paglakad sa gabi na walang kasama.){/i}"

# game/logic/events/giant.rpy:111
translate philippine giant_1_label_e3e69c43:

    # ch_Name "{i}(I was lucky he was stopped by my feeble combat spell.){/i}"
    ch_Name "{i}(At napigilan ko siya gamit ng mahinang mahika.){/i}"

# game/logic/events/giant.rpy:113
translate philippine giant_1_label_0e9a9852:

    # ch_Name "{i}(Damn, that was even too close...){/i}"
    ch_Name "{i}(Bwisit, malapit na 'yun...){/i}"

# game/logic/events/giant.rpy:114
translate philippine giant_1_label_185a728d:

    # ch_Name "{i}(His paws would have easily broken my bones.){/i}"
    ch_Name "{i}(Madaling mabali ang aking mga buto sa kanya.){/i}"

# game/logic/events/giant.rpy:115
translate philippine giant_1_label_d746b2a1:

    # ch_Name "{i}(I need to lie down...){/i}"
    ch_Name "{i}(Hihiga muna ako...){/i}"

# game/logic/events/giant.rpy:126
translate philippine giant_1_label_5205b150:

    # ch_Name "{i}(I hope I don't regret it...){/i}"
    ch_Name "{i}(Sana hindi ito ang maling desisyon...){/i}"

# game/logic/events/giant.rpy:127
translate philippine giant_1_label_e09d8bf5:

    # ch_Name "All right, all right! I'll help."
    ch_Name "Sige, sige! Tutulong ako."

# game/logic/events/giant.rpy:129
translate philippine giant_1_label_c17b209e:

    # Giant "Y-you... Really?"
    Giant "I-ikaw... Talaga?"

# game/logic/events/giant.rpy:130
translate philippine giant_1_label_24bd4a7b:

    # Giant "Argh... You're a good man..."
    Giant "Argh... Mabuti kang tao..."

# game/logic/events/giant.rpy:132
translate philippine giant_1_label_f5f5ed9d:

    # Giant "Words are hard for me. Talking..."
    Giant "Mahirap magsalita. Magsalita..."

# game/logic/events/giant.rpy:133
translate philippine giant_1_label_aab5c872:

    # Giant "I need {b}a potion{/b}... To {b}lift curses{/b}."
    Giant "Kailangan ko ng {b}potion{/b}... Para {b}paalisin ang sumpa{/b}."

# game/logic/events/giant.rpy:134
translate philippine giant_1_label_ee22f82d:

    # Giant "Sabrina..."
    Giant ""

# game/logic/events/giant.rpy:136
translate philippine giant_1_label_f06f5087:

    # ch_Name "Do you know Miss Spellman?"
    ch_Name "Kilala mo ba si Miss Spellman?"

# game/logic/events/giant.rpy:137
translate philippine giant_1_label_0e23c020:

    # ch_Name "{i}(Something's going on here.){/i}"
    ch_Name "{i}(May problema talaga dito.){/i}"

# game/logic/events/giant.rpy:138
translate philippine giant_1_label_f69ff33a:

    # ch_Name "I'll talk to Sabrina and try to help."
    ch_Name "Uusapin ko si Sabrina at hihingi ng tulong."

# game/logic/events/giant.rpy:140
translate philippine giant_1_label_7bbf7244:

    # Giant "I'll be... argh... waiting!"
    Giant "Ako ay... argh... maghihintay sa iyo!"

# game/logic/events/giant.rpy:141
translate philippine giant_1_label_23617545:

    # ch_Name "{i}(That was pretty weird...){/i}"
    ch_Name "{i}(Ano ba ang nangyari?){/i}"

# game/logic/events/giant.rpy:143
translate philippine giant_1_label_5d0ef032:

    # ch_Name "{i}(I've had enough adventures for one night. Gonna sleep it off.){/i}"
    ch_Name "{i}(Sapat na ito para sa aking pakikipagsapalaram ngayong gabi. Matutulog na ako.){/i}"

# game/logic/events/giant.rpy:158
translate philippine Giant_2_label_3637bde0:

    # ch_Name "{i}(Sabrina seems to be in good spirits.){/i}"
    ch_Name "{i}(Parang masaya si Sabrina.){/i}"

# game/logic/events/giant.rpy:159
translate philippine Giant_2_label_3e938d2c:

    # ch_Name "{i}(Now would be an excellent time to inquire about a potion for that giant in town.){/i}"
    ch_Name "{i}(Mabuti na itong panahon para humingi ng potion para sa higante sa Dale.){/i}"

# game/logic/events/giant.rpy:160
translate philippine Giant_2_label_1a458728:

    # ch_Name "Miss Spellman, may I address you?"
    ch_Name "Ms. Spellman, pwede bang magtanong sa iyo?"

# game/logic/events/giant.rpy:162
translate philippine Giant_2_label_91afc8d1:

    # Sabrina "Of course, [Name]!"
    Sabrina "Siyempre, [Name]!"

# game/logic/events/giant.rpy:163
translate philippine Giant_2_label_4e3f09bb:

    # Sabrina "But feel free to ask questions in class too, okay?"
    Sabrina "Pero huwag kang mahiyang magtanong sa klase, okay?"

# game/logic/events/giant.rpy:164
translate philippine Giant_2_label_f2ca5068:

    # ch_Name "I wasn't embarrassed."
    ch_Name "Hindi ako nahihiya."

# game/logic/events/giant.rpy:165
translate philippine Giant_2_label_f96ffb9e:

    # ch_Name "It's just not about studying."
    ch_Name "Pero hindi ito tungkol sa klase natin."

# game/logic/events/giant.rpy:166
translate philippine Giant_2_label_44d2a5d6:

    # ch_Name "It's more about magical creatures."
    ch_Name "Tungkol naman sa mga mahikang hayop ang tanong ko."

# game/logic/events/giant.rpy:168
translate philippine Giant_2_label_e7ac2dff:

    # Sabrina "Creatures?"
    Sabrina "Hayop?"

# game/logic/events/giant.rpy:169
translate philippine Giant_2_label_1d20dfde:

    # ch_Name "I recently came across a strange giant in Dale."
    ch_Name "May nakita ako na nakakaibang higante sa Dale."

# game/logic/events/giant.rpy:170
translate philippine Giant_2_label_c054f392:

    # Sabrina "A giant? You intrigue me."
    Sabrina "Higante? Ano pa?"

# game/logic/events/giant.rpy:171
translate philippine Giant_2_label_5ea8eb34:

    # Sabrina "And what would you like to know about it?"
    Sabrina "At ano ang gusto mo malaman?"

# game/logic/events/giant.rpy:173
translate philippine Giant_2_label_c6d0cfa3:

    # ch_Name "You wouldn't happen to have a curse-relieving potion, would you?"
    ch_Name "May curse-relieving potion ka ba?"

# game/logic/events/giant.rpy:174
translate philippine Giant_2_label_92bf47e4:

    # Sabrina "You're running ahead of the course, [Name]."
    Sabrina "Nauna ka sa mga kurso mo, [Name]."

# game/logic/events/giant.rpy:175
translate philippine Giant_2_label_29c252f3:

    # Sabrina "You won't be studying it until the end of the school year."
    Sabrina "Pag-aaralan mo 'yan sa dulo ng taon."

# game/logic/events/giant.rpy:177
translate philippine Giant_2_label_969a581a:

    # Sabrina "Why would you want to do that?"
    Sabrina "Bakit gusto mo ang potion na 'yan?"

# game/logic/events/giant.rpy:178
translate philippine Giant_2_label_b4c39f7b:

    # Sabrina "You haven't cursed anyone, have you?"
    Sabrina "May sinumpa ka ba?"

# game/logic/events/giant.rpy:179
translate philippine Giant_2_label_421f7484:

    # ch_Name "No, that's... It's not for me."
    ch_Name "Hindi, para... Hindi para sa akin."

# game/logic/events/giant.rpy:180
translate philippine Giant_2_label_7c52f3ae:

    # Sabrina "Who's it for?"
    Sabrina "Para kanino?"

# game/logic/events/giant.rpy:181
translate philippine Giant_2_label_22ee6288:

    # ch_Name "Oh, it's a strange story..."
    ch_Name "Hindi mo maniniwalaan ang kuwentong ito..."

# game/logic/events/giant.rpy:183
translate philippine Giant_2_label_ef599765:

    # Sabrina "All the more interesting. Spit it out."
    Sabrina "Sabihin mo na nga."

# game/logic/events/giant.rpy:184
translate philippine Giant_2_label_433b5c01:

    # ch_Name "One drawl in Dale, some ugly giant in rags came at me."
    ch_Name "May higante na nag-usap sa akin."

# game/logic/events/giant.rpy:185
translate philippine Giant_2_label_839e70ec:

    # ch_Name "He was begging for a potion to break the curse."
    ch_Name "Humingi siya ng potion para matanggal ang sumpa sa kanya."

# game/logic/events/giant.rpy:186
translate philippine Giant_2_label_351719e2:

    # Sabrina "Um... Anything else?"
    Sabrina "Um... meron pa ba?"

# game/logic/events/giant.rpy:187
translate philippine Giant_2_label_70206e65:

    # ch_Name "He spoke your name..."
    ch_Name "Sinabi niya ang pangalan mo..."

# game/logic/events/giant.rpy:188
translate philippine Giant_2_label_08e0b55d:

    # ch_Name "Unintelligible, but I got it..."
    ch_Name "Mahirap maintindihan ang sinasabi niya..."

# game/logic/events/giant.rpy:190
translate philippine Giant_2_label_a4dba615:

    # Sabrina "HMM... My name?"
    Sabrina "HMMM... Pangalan ko?"

# game/logic/events/giant.rpy:191
translate philippine Giant_2_label_5bd55916:

    # Sabrina "I wonder how he knows it..."
    Sabrina "Bakit niya alam ang pangalan ko..."

# game/logic/events/giant.rpy:192
translate philippine Giant_2_label_8fdc573d:

    # Sabrina "Could it be him?.. No, nonsense!"
    Sabrina "Siya nga ba?... Hindi, imposible!"

# game/logic/events/giant.rpy:193
translate philippine Giant_2_label_2444346b:

    # ch_Name "{i}(I wonder who she's talking about?){/i}"
    ch_Name "{i}(Sino ba ang \"siya\"?){/i}"

# game/logic/events/giant.rpy:195
translate philippine Giant_2_label_7acde20c:

    # Sabrina "We need to be sure. I'll give you the right potion."
    Sabrina "Dapat sigurado tayo. Ibibigay ko sa iyo ang potion."

# game/logic/events/giant.rpy:196
translate philippine Giant_2_label_e2b2ff66:

    # ch_Name "You will?"
    ch_Name "Talaga?"

# game/logic/events/giant.rpy:197
translate philippine Giant_2_label_e26e6f8a:

    # Sabrina "Of course, I will. But only on the condition that you tell me everything."
    Sabrina "Talaga. Pero dapat sasabihin mo sa akin ang lahat."

# game/logic/events/giant.rpy:198
translate philippine Giant_2_label_a689c1e0:

    # Sabrina "I'll just take a second to see if there are any potions ready."
    Sabrina "Tignan ko lang kung may potion na ako na nahanda."

# game/logic/events/giant.rpy:199
translate philippine Giant_2_label_41544636:

    # Sabrina "Curse breakers, curse breakers..."
    Sabrina ""

# game/logic/events/giant.rpy:201
translate philippine Giant_2_label_846971fa:

    # Sabrina "Oh, of course, there it is!"
    Sabrina "Siyempre, ito pala!"

# game/logic/events/giant.rpy:202
translate philippine Giant_2_label_bffa0bca:

    # Sabrina "How could I miss it?"
    Sabrina "Bakit hindi ko nakita?"

# game/logic/events/giant.rpy:204
translate philippine Giant_2_label_6dd3d436:

    # Sabrina "Here you go."
    Sabrina "Heto!"

# game/logic/events/giant.rpy:205
translate philippine Giant_2_label_3ab59d17:

    # ch_Name "Thank you, Miss Spellman."
    ch_Name "Salamat, Ms. Spellman."

# game/logic/events/giant.rpy:206
translate philippine Giant_2_label_6ba3d9b7:

    # ch_Name "I knew I could count on you."
    ch_Name "Matitiwalaan ka talaga."

# game/logic/events/giant.rpy:207
translate philippine Giant_2_label_09922e76:

    # Sabrina "I hope it helps that giant you were talking about."
    Sabrina "Sana matutulungan mo ang higante sa sinasabi mo."

# game/logic/events/giant.rpy:209
translate philippine Giant_2_label_ca939a29:

    # ch_Name "So do I. I have to run, thank you."
    ch_Name "Oo. Aalis na ako, salamat."

# game/logic/events/giant.rpy:210
translate philippine Giant_2_label_8deaf9fc:

    # Sabrina "Bye-bye."
    Sabrina ""

# game/logic/events/giant.rpy:213
translate philippine Giant_2_label_b66c15ab:

    # ch_Name "{i}(Alright, that's it for today with lessons. Now I'm free.){/i}"
    ch_Name "{i}(Tapos na ang mga klase ko ngayon.){/i}"

# game/logic/events/giant.rpy:214
translate philippine Giant_2_label_73109193:

    # ch_Name "{i}(I should probably get Elijah his nectar. He's waiting for it.){/i}"
    ch_Name "{i}(Siguro kunin ko ang nectar ni Elijah. Naghihintay siya.){/i}"

# game/logic/events/giant.rpy:219
translate philippine Giant_2_label_3a0d59fa:

    # ch_Name "{i}(He's always somewhere around here in the evening...){/i}"
    ch_Name "{i}(Nandito naman siya kapag gabi na.){/i}"

# game/logic/events/giant.rpy:221
translate philippine Giant_2_label_260c6984:

    # ch_Name "{i}(Here he is!){/i}"
    ch_Name "{i}(Ayan!){/i}"

# game/logic/events/giant.rpy:222
translate philippine Giant_2_label_47fc0b1c:

    # ch_Name "{i}(It looks like he's been waiting for me.){/i}"
    ch_Name "{i}(Parang naghintay siya para sa akin.){/i}"

translate philippine strings:

    # game/logic/events/giant.rpy:11
    old "0 Come closer"
    new "0 Lumapit"

    # game/logic/events/giant.rpy:11
    old "Stay away"
    new "Lumayo"

    # game/logic/events/giant.rpy:83
    old "Say you'll help"
    new "Tulungan siya"

    # game/logic/events/giant.rpy:83
    old "Don't help"
    new "Huwag tumulong"

