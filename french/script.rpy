# TODO: Translation updated at 2022-01-25 10:31

# game/script.rpy:217
translate french 69ff737f:

    # "Here you can keep track of all the routes, activities, etc. that are available to you. and quickly navigate to the desired one, without wasting time walking around the locations and searching. Next to the avatars of the characters are icons, the decoding of which is next."
    "Ici, vous pouvez garder une trace de tous les itinéraires, activités etc.. qui seront à votre disposition. Naviguez rapidement vers celui que vous souhaitez, sans perdre de temps à parcourir les lieux et à chercher. À côté des avatars des personnages se trouvent des icônes, dont le décodage est le suivant."

# game/script.rpy:742
translate french start_54c881dc:

    # NoName "Hi! My name is..."
    NoName "Hey, je m'appelle..."

# game/script.rpy:768
translate french start_input_2_a0ecd7a8:

    # ch_Name "My name is [Name] [Surname] and I am 18 years old."
    ch_Name "Je m'appelle [Name] [Surname] et j'ai 18 ans."

# game/script.rpy:774
translate french start_input_2_f789cadc:

    # ch_Name "I should have been at training camp with my team right now."
    ch_Name "J'devrais être normalement au camp d'entraînement avec mon équipe en ce moment."

# game/script.rpy:776
translate french start_input_2_d05c9319:

    # ch_Name "But fate had something else prepared for me."
    ch_Name "Mais le destin m'a réservé autre chose.."

# game/script.rpy:780
translate french start_input_2_8ba3057e:

    # ch_Name "Double open fracture..."
    ch_Name "Double fracture ouverte..."

# game/script.rpy:781
translate french start_input_2_9ca377b3:

    # ch_Name "It's a miracle I'm not crippled."
    ch_Name "C'est un miracle que je ne sois pas paralysé."

# game/script.rpy:782
translate french start_input_2_b118fdd0:

    # ch_Name "Still, I had to say goodbye to my soccer career..."
    ch_Name "En contrepartie, j'devrais dire au revoir à ma carrière de footballeur..."

# game/script.rpy:783
translate french start_input_2_e826d333:

    # ch_Name "And to the scholarship I needed for college as well."
    ch_Name "Et à cette bourse dont j'avais besoin pour l'université aussi."

# game/script.rpy:787
translate french start_input_2_9cc3f3ac:

    # ch_Name "So I had to find another way to get an education."
    ch_Name "Du coup j'ai dû trouver un autre moyen d'obtenir ma formation."

# game/script.rpy:789
translate french start_input_2_55620e58:

    # ch_Name "Samantha suggested I try photography."
    ch_Name "Samantha m'avait suggéré d'essayer la photographie"

# game/script.rpy:791
translate french start_input_2_a8a5cb72:

    # ch_Name "As it turns out, I'm a natural!"
    ch_Name "Il s'avère que j'suis naturel !"

# game/script.rpy:793
translate french start_input_2_e0b7f024:

    # ch_Name "Now I work part-time as a photographer to save money for college."
    ch_Name "Et maintenant, j'travaille à temps partiel comme photographe pour économiser de l'argent pour l'université."

# game/script.rpy:798
translate french start_input_2_0eb03f84:

    # ch_Name "I've been applying filters on photos all day..."
    ch_Name "J'ai appliqué des filtres sur les photos toute la journée..."

# game/script.rpy:800
translate french start_input_2_86ff8bea:

    # ch_Name "I hope the client will be happy."
    ch_Name "J'espère que le client sera ravi."

# game/script.rpy:802
translate french start_input_2_19d3c2fb:

    # ch_Name "A few more shoots and I'll have enough for a down payment."
    ch_Name "Encore quelques shooting et j'aurai assez d'argent pour un acompte."

# game/script.rpy:804
translate french start_input_2_b9fa1026:

    # ch_Name "And I still have time to get out of this shit-hole this semester."
    ch_Name "Et j'ai encore le temps de sortir de cette merde ce semestre."

# game/script.rpy:818
translate french start_input_2_b6ce45a5:

    # NoName "Knock-Knock"
    NoName "Toc toc"

# game/script.rpy:863
translate french start_input_2_6f8b77bd:

    # Olivia "[Name] [Surname]! You've been slacking again?"
    Olivia "[Name] [Surname]! Tu t'es encore relâché ?"

# game/script.rpy:865
translate french start_input_2_fcb030eb:

    # Olivia "I expected as much from you."
    Olivia "J'en attendais pas moins de toi."

# game/script.rpy:867
translate french start_input_2_944e7bfb:

    # ch_Name "Please don't start that, Olivia, I've been working all day."
    ch_Name "S'il te plaît Olivia, commence pas, j'ai bossé toute la journée."

# game/script.rpy:869
translate french start_input_2_9039396f:

    # Olivia "Start what?!"
    Olivia "Commencer quoi ?!"

# game/script.rpy:871
translate french start_input_2_f6f68885:

    # Olivia "What kind of tone is that?"
    Olivia "En quel ton tu me parles ?"

# game/script.rpy:873
translate french start_input_2_4d4ef9ab:

    # Olivia "You've been in your room all day!"
    Olivia "T'as resté dans ta chambre toute la journée !"

# game/script.rpy:875
translate french start_input_2_3c898584:

    # Olivia "Or do you think I'm blind?!"
    Olivia "Tu crois que j'suis aveugle ?!"

# game/script.rpy:877
translate french start_input_2_b510ad9f:

    # ch_Name "But..."
    ch_Name "Mais..."

# game/script.rpy:879
translate french start_input_2_ad8c35fe:

    # Olivia "No buts!"
    Olivia "Pas de mais qui tienne !"

# game/script.rpy:881
translate french start_input_2_9a44f10c:

    # Olivia "That's what Don was talking about. You're out of control!"
    Olivia "C'est de cela que parlait Don. Tu te contrôles plus !"

# game/script.rpy:883
translate french start_input_2_3a73df72:

    # Olivia "Your ingratitude leaves us no choice."
    Olivia "Ton ingratitude me laisse pas le choix."

# game/script.rpy:885
translate french start_input_2_2a5612ed:

    # Olivia "If your mother heard that at 18 years old..."
    Olivia "Si ta mère entendait ça à 18 ans..."

# game/script.rpy:887
translate french start_input_2_362faf0b:

    # Olivia "...her son would neither study nor work!"
    Olivia "...que son fils n'étudie et travaille pas !"

# game/script.rpy:889
translate french start_input_2_2043c397:

    # Olivia "She'd be spinning in her grave!"
    Olivia "Elle se retournerait dans sa tombe !"

# game/script.rpy:891
translate french start_input_2_f6a83891:

    # ch_Name "Olivia..."
    ch_Name "Olivia..."

# game/script.rpy:893
translate french start_input_2_117ab70a:

    # ch_Name "Leave my mother alone!"
    ch_Name "Laisse ma mère tranquille !"

# game/script.rpy:895
translate french start_input_2_ce40e133:

    # Olivia "Don't you raise your voice at me in my house!"
    Olivia "N'hausse pas le ton contre moi dans ma maison !"

# game/script.rpy:897
translate french start_input_2_532196a0:

    # Olivia "Truth hurts, doesn't it?"
    Olivia "La vérité fait mal, pas vrai ?"

# game/script.rpy:899
translate french start_input_2_f6724a46:

    # ch_Name "You know why this is the way it is!"
    ch_Name "Tu sais déjà pourquoi c'est comme ça !"

# game/script.rpy:901
translate french start_input_2_4682ac22:

    # ch_Name "I'd be at university right now if it wasn't for my leg..."
    ch_Name "J'serais déjà à l'université en ce moment si ce n'était pas pour ma jambe..."

# game/script.rpy:903
translate french start_input_2_6eb4f2dc:

    # Olivia "You should have thought of that before!"
    Olivia "T'aurais dû y penser avant !"

# game/script.rpy:905
translate french start_input_2_245150b8:

    # Olivia "Sport is always a risk."
    Olivia "Le sport sera toujours un risque."

# game/script.rpy:907
translate french start_input_2_5783fa11:

    # Olivia "You should've done less loafing and studied more. Like Ashley!"
    Olivia "T'aurais dû moins flâner et étudier plus. Comme Ashley !"

# game/script.rpy:909
translate french start_input_2_aedee487:

    # Olivia "And now what?"
    Olivia "Et maintenant ?"

# game/script.rpy:911
translate french start_input_2_036404a6:

    # Olivia "How do you plan to get an education?"
    Olivia "T'as un plan pour continuer ta formation ?"

# game/script.rpy:913
translate french start_input_2_b1e52819:

    # ch_Name "I'm working on it."
    ch_Name "J'y travaille"

# game/script.rpy:915
translate french start_input_2_2aa9b9d5:

    # Olivia "He's working..."
    Olivia "T'y travailles..."

# game/script.rpy:917
translate french start_input_2_b33dc905:

    # Olivia "I see how hard you’re \"working\"!"
    Olivia "J'vois a quel point t'y \"travailles\"!"

# game/script.rpy:919
translate french start_input_2_691c3787:

    # Olivia "[Name], our hospitality is not unlimited."
    Olivia "[Name], notre amabilité n'est pas infini."

# game/script.rpy:921
translate french start_input_2_2089379b:

    # ch_Name "{i}(That's for sure...){/i}"
    ch_Name "{i}(Ça c'est sûr...){/i}"

# game/script.rpy:923
translate french start_input_2_2869137c:

    # Olivia "We didn't want this, but you leave us no choice."
    Olivia "On ne voualait pas en arriver là, mais tu nous laisses pas le choix."

# game/script.rpy:925
translate french start_input_2_42f41d8b:

    # Olivia "If you want to throw your life away..."
    Olivia "Si tu veux gâcher ta vie..."

# game/script.rpy:927
translate french start_input_2_b962a7e0:

    # Olivia "We won't let you do it with your feet dangling off Don's neck!"
    Olivia "On te laissera pas faire avec tes pieds suspendus au cou de Don !"

# game/script.rpy:929
translate french start_input_2_f6f5841f:

    # Olivia "You're a grown man."
    Olivia "T'es un adulte"

# game/script.rpy:931
translate french start_input_2_bca2c05c:

    # Olivia "You can take care of yourself."
    Olivia "Tu peux prendre soin de toi"

# game/script.rpy:933
translate french start_input_2_f4cb3c7a:

    # ch_Name "What do you mean? Are you kicking me out?"
    ch_Name "Ou tu veux en venir ? Tu veux me mettre dehors ?"

# game/script.rpy:935
translate french start_input_2_90f3a346:

    # ch_Name "But, Olivia, come on... Where am I supposed to go?"
    ch_Name "Mais, Olivia, vas-y... Tu veux que j'aille ou ?"

# game/script.rpy:937
translate french start_input_2_2661214d:

    # Olivia "What do you take us for? Savages?"
    Olivia "Tu nous prends pour qui ? Des sauvages ?"

# game/script.rpy:939
translate french start_input_2_3c004877:

    # Olivia "You're my best friend's son, God rest her soul."
    Olivia "T'es le fils de ma meilleure amie, que Dieu accorde la paix à son âme."

# game/script.rpy:941
translate french start_input_2_668cf83e:

    # Olivia "And I talked Don out of kicking you out."
    Olivia "Et j'ai dissuadé Don de pas te virer."

# game/script.rpy:943
translate french start_input_2_cbbc98eb:

    # ch_Name "Thank you, I knew..."
    ch_Name "Merci, j'sais..."

# game/script.rpy:945
translate french start_input_2_53623f8c:

    # Olivia "But!"
    Olivia "Mais !"

# game/script.rpy:947
translate french start_input_2_7e4ac717:

    # ch_Name "{i}(Of course there’s a \"but\"!){/i}"
    ch_Name "{i}(Bien évidemment y'a un \"mais\"!){/i}"

# game/script.rpy:949
translate french start_input_2_09453d86:

    # Olivia "We're done supporting you."
    Olivia "On te supportera plus comme avant."

# game/script.rpy:951
translate french start_input_2_a2fcf8a5:

    # ch_Name "What?!"
    ch_Name "Quoi ?!"

# game/script.rpy:953
translate french start_input_2_eb708d4a:

    # Olivia "From now on, if you want to live here..."
    Olivia "A partir de maintenant, si tu veux vivre ici..."

# game/script.rpy:955
translate french start_input_2_1394e177:

    # Olivia "Pay your rent as a lodger."
    Olivia "Faudra payer ton loyer comme un locataire"

# game/script.rpy:957
translate french start_input_2_306af026:

    # ch_Name "What?! What?!"
    ch_Name "Quoi ?! Quoi ?!"

# game/script.rpy:959
translate french start_input_2_97dd3450:

    # Olivia "Pay your rent as a lodger."
    Olivia "Faudra payer ton loyer comme un locataire"

# game/script.rpy:961
translate french start_input_2_8ac87752:

    # ch_Name "You're kidding, right?"
    ch_Name "Tu t'moques de moi, pas vrai ?"

# game/script.rpy:963
translate french start_input_2_833ce195:

    # Olivia "You don't have to pay for August, but..."
    Olivia "T'auras pas à payer pour le mois d'août mais..."

# game/script.rpy:965
translate french start_input_2_db8f91ff:

    # ch_Name "Olivia, hey. Tell me you're kidding..."
    ch_Name "Oh Olivia, dis moi que c'est une blague..."

# game/script.rpy:967
translate french start_input_2_3da42f20:

    # Olivia "I need your first and last month's payment in September."
    Olivia "J'aurai besoin que tu payes le premier et le dernier jour du mois de septembre."

# game/script.rpy:969
translate french start_input_2_a680031d:

    # Olivia "Plus a security deposit in case of property damage."
    Olivia "Plus un dépôt de garantie en cas de dommages matériels."

# game/script.rpy:971
translate french start_input_2_b863e1f9:

    # ch_Name "What... what kind of nonsense is that?!"
    ch_Name "Qu'est-ce que... C'est quel genre d'absurdité ça ?!"

# game/script.rpy:973
translate french start_input_2_aa82ecc0:

    # ch_Name "How am I supposed to save for school while paying rent? It's a vicious circle!"
    ch_Name "Comment suis-je censé économiser pour l'école tout en payant un loyer ? C'est un cercle vicieux !"

# game/script.rpy:975
translate french start_input_2_a889e973:

    # Olivia "Young man!"
    Olivia "Jeune homme"

# game/script.rpy:977
translate french start_input_2_419bb65f:

    # Olivia "You're under my roof."
    Olivia "T'es sous mon toit"

# game/script.rpy:979
translate french start_input_2_b59257ff:

    # Olivia "You follow my rules."
    Olivia "Tu suis mes règles"

# game/script.rpy:981
translate french start_input_2_0c3ee8a6:

    # Olivia "You don't like our offer?"
    Olivia "Tu n'aimes pas mon offre ?"

# game/script.rpy:983
translate french start_input_2_143d02b4:

    # Olivia "Go into the living room and say it to Don's face."
    Olivia "Va dans le salon et dis le en face de Don."

# game/script.rpy:985
translate french start_input_2_82ed18d6:

    # Olivia "Or do you only have the courage to bicker with me?"
    Olivia "Ou t'as le courage de te chamailler avec moi seulement ?"

# game/script.rpy:987
translate french start_input_2_b8f6f104:

    # ch_Name "{i}(Damn it, I knew I shouldn't have fallen for her provocations...){/i}"
    ch_Name "{i}(Merde, je savais que je n'aurais pas dû tomber dans ses provocations...){/i}"

# game/script.rpy:989
translate french start_input_2_931280c0:

    # ch_Name "All right. I'll talk to Don."
    ch_Name "Très bien. Je vais en parler à Don."

# game/script.rpy:991
translate french start_input_2_512eba66:

    # ch_Name "Maybe he won't be so deaf to reason."
    ch_Name "Peut-être ne sera-t-il pas si sourd à la raison."

# game/script.rpy:993
translate french start_input_2_fe2acc96:

    # Olivia "Good luck with that."
    Olivia "Bonne chance pour ça."

# game/script.rpy:995
translate french start_input_2_86f4b2fc:

    # Olivia "After all, it was his idea."
    Olivia "Après tout, c'était son idée."

translate french strings:

    # game/script.rpy:61
    old "Nice"
    new "Agréable"

    # game/script.rpy:63
    old "Flirty"
    new "Dragueur"

    # game/script.rpy:65
    old "Troll"
    new "Troll"

    # game/script.rpy:67
    old "Angry"
    new "En colère"

    # game/script.rpy:69
    old "Flirt"
    new "Flirter"

    # game/script.rpy:71
    old "Support"
    new "Supporter"

    # game/script.rpy:73
    old "Joke"
    new "Plaisanter"

    # game/script.rpy:75
    old "Funny"
    new "Marrant"

    # game/script.rpy:77
    old "Sad"
    new "Triste"

    # game/script.rpy:79
    old "Pessimistic"
    new "Pessimiste"

    # game/script.rpy:81
    old "Fun"
    new "Amusant"

    # game/script.rpy:84
    old "Lesser Heal"
    new "Faible soin"

    # game/script.rpy:86
    old "Stoneskin"
    new "Peau de pierre"

    # game/script.rpy:88
    old "Inspiration"
    new "Inspiration"

    # game/script.rpy:90
    old "Magic Reflection"
    new "Réflexion magique"

    # game/script.rpy:93
    old "CALENDAR"
    new "CALENDRIER"

    # game/script.rpy:95
    old "CHARACTER INFO"
    new "INFO DU PERSONNAGE"

    # game/script.rpy:97
    old "MY PROFILE"
    new "MON PROFIL"

    # game/script.rpy:100
    old "Makeup bag"
    new "Trousse de maquillage"

    # game/script.rpy:102
    old "Money"
    new "Argent"

    # game/script.rpy:104
    old "Documents"
    new "Documents"

    # game/script.rpy:106
    old "Condoms"
    new "Préservatifs"

    # game/script.rpy:108
    old "Book"
    new "Livre"

    # game/script.rpy:110
    old "Corridor"
    new "Couloir"

    # game/script.rpy:112
    old "Living Room"
    new "Salon"

    # game/script.rpy:114
    old "Girls Room 1"
    new "Chambres des filles 1"

    # game/script.rpy:116
    old "Girls Room 2"
    new "Chambre des filles 2"

    # game/script.rpy:118
    old "My Room"
    new "Ma chambre"

    # game/script.rpy:120
    old "Sleep"
    new "Dormir"

    # game/script.rpy:122
    old "Inner Garden"
    new "Jardin intérieur"

    # game/script.rpy:124
    old "Entrance"
    new "Entrée"

    # game/script.rpy:126
    old "Left Wing"
    new "Aile gauche"

    # game/script.rpy:128
    old "Right Wing"
    new "Aile droite"

    # game/script.rpy:130
    old "Main Hall"
    new "Hall principal"

    # game/script.rpy:132
    old "Library"
    new "Bibliothèque"

    # game/script.rpy:134
    old "Classroom 1"
    new "Salle de classe 1"

    # game/script.rpy:136
    old "Cafe"
    new "Café"

    # game/script.rpy:138
    old "Classroom 2"
    new "Salle de classe 2"

    # game/script.rpy:140
    old "Bathroom"
    new "Salle de bain"

    # game/script.rpy:142
    old "Elijah's Room"
    new "Chambre de Elijah"

    # game/script.rpy:144
    old "Outside"
    new "Dehors"

    # game/script.rpy:146
    old "Upstairs"
    new "A l'étage"

    # game/script.rpy:148
    old "Sorting Book"
    new "Tri de livre"

    # game/script.rpy:150
    old "Ashley's Bedroom"
    new "Chambre d'Ashley"

    # game/script.rpy:152
    old "Samantha's Bedroom"
    new "Chambre de Samantha"

    # game/script.rpy:154
    old "Downstairs"
    new "En bas"

    # game/script.rpy:156
    old "My Bedroom"
    new "Ma chambre"

    # game/script.rpy:158
    old "Olivia's Bedroom"
    new "Chambre d'Olivia"

    # game/script.rpy:160
    old "Cupboard"
    new "Armoire"

    # game/script.rpy:162
    old "Olivia’s Purse"
    new "Sac à main d'Olivia"

    # game/script.rpy:164
    old "Photo"
    new "Photo"

    # game/script.rpy:166
    old "Painting"
    new "Peinture"

    # game/script.rpy:168
    old "Bath Ducks"
    new "Canard de bain"

    # game/script.rpy:170
    old "Toothbrush"
    new "Brosse à dent"

    # game/script.rpy:172
    old "Viewpoint"
    new "Point de vue"

    # game/script.rpy:174
    old "Main Street"
    new "Rue principale"

    # game/script.rpy:176
    old "Coffee Hut"
    new "Bar"

    # game/script.rpy:178
    old "Alley"
    new "Ruelle"

    # game/script.rpy:180
    old "Club"
    new "Club"

    # game/script.rpy:182
    old "Hotel"
    new "Hotel"

    # game/script.rpy:184
    old "Shop"
    new "Magasin"

    # game/script.rpy:186
    old "Spa"
    new "Spa"

    # game/script.rpy:188
    old "Exit"
    new "Sortie"

    # game/script.rpy:190
    old "Room"
    new "Pièce"

    # game/script.rpy:192
    old "Campus"
    new "Campus"

    # game/script.rpy:194
    old "Academy"
    new "Académie"

    # game/script.rpy:196
    old "Dale"
    new "Vallée"

    # game/script.rpy:206
    old "All characters are 18 age or older even if specified otherwise."
    new "Tous les personnages ont 18 ans ou plus, même si ils sont spécifier autrement."

    # game/script.rpy:207
    old "This game contains violence, obscene scenes (naked girls and sex scenes), "
    new "Ce jeu contient de la violence, des scènes obscènes (filles nues et scènes de sexe),"

    # game/script.rpy:208
    old "rude humor, scenes that depict smoking or infrequent use of profanity."
    new "humour grossier, scènes illustrant le tabagisme ou l'utilisation peu fréquente du blasphèmes."

    # game/script.rpy:330
    old "Warning!"
    new "Attention !"

    # game/script.rpy:331
    old "In order to maximize the enjoyment of the game we have added music and ambient sounds, including those of an erotic nature."
    new "Afin de maximiser le plaisir du jeu, nous avons ajouté de la musique et des sons ambiants, y compris ceux de nature érotique."

    # game/script.rpy:332
    old "You can adjust the volume and turn off the music in the settings."
    new "Vous pouvez régler le volume et désactiver la musique dans les paramètres."

    # game/script.rpy:333
    old "We hope this did not cause you any trouble."
    new "Nous espérons que cela ne vous causera aucun problème."

    # game/script.rpy:334
    old "Enjoy the game!"
    new "Amusez vous bien !"

    # game/script.rpy:340
    old "Note that our game features LGBTQ+ scenes. If you're not interested in stuff like that at all, you can turn this option"
    new "Notez que notre jeu propose des scènes LGBTQ+. Si vous n'êtes pas du tout intéressé par ce genre de choses, vous pouvez activer cette option"

    # game/script.rpy:340
    old "off right now (you can always turn it on later, in the settings). Leave this option enabled if you're only"
    new "Désactiver (vous pouvez toujours l'activer plus tard, dans les paramètres). Laissez cette option activée si vous êtes seulement"

    # game/script.rpy:340
    old "interested in some of the possible content. Don't worry! We'll ask you beforehand each"
    new "intéressé par certains des contenus possibles. Ne vous inquiétez pas ! Nous vous demanderons au préalable chaque"

    # game/script.rpy:340
    old "time there's a chance to encounter such interactions."
    new "fois il y a une chance de rencontrer de telles interactions."

    # game/script.rpy:340
    old "Do you want to enable LGBTQ+ content?"
    new "Voulez-vous activer le contenu LGBTQ+ ?"

    # game/script.rpy:351
    old "Enable"
    new "Activer"

    # game/script.rpy:352
    old "Disable"
    new "Désactiver"

    # game/script.rpy:356
    old "On Sunday evening, all students return to Cordale. So you did."
    new "Le dimanche soir, tous les élèves retournent à Cordale. Donc tu l'as fait."

    # game/script.rpy:728
    old "{b}Olivia{/b} said I have to talk to Don about this rent thing. \nHe should be in the living room."
    new "{b}Olivia{/b} a dit que j'devais parler à Don de cette histoire de loyer. \nIl devrait être dans le salon."

    # game/script.rpy:748
    old "Enter your name"
    new "Entrez votre nom"

    # game/script.rpy:748
    old "James"
    new "James"

    # game/script.rpy:759
    old "Enter your surname"
    new "Entrez votre nom de famille"

    # game/script.rpy:759
    old "Pogger"
    new "Pogger"

    # game/script.rpy:807
    old "Continue"
    new "Continue"

    # game/script.rpy:807
    old "Skip prologue"
    new "Passer"

