# TODO: Translation updated at 2022-01-30 21:40

# game/logic/locations/ashlie_room.rpy:8
translate hungarian ashlie_room_label_457043ad:

    # ch_Name "{i}(Ashley's probably in the middle of the training session. I don't want to bother her.){/i}"
    ch_Name "{i}(Ashely biztosan egy éjjeli edzés közepén van. Nem akarom zavarni őt.){/i}"

# game/logic/locations/ashlie_room.rpy:14
translate hungarian ashlie_room_label_67e55910:

    # ch_Name "{i}(Ashley's usually training around this time.){/i}"
    ch_Name "{i}(Ashley általában ebben az időpontban szokott edzeni.){/i}"

# game/logic/locations/ashlie_room.rpy:15
translate hungarian ashlie_room_label_18dd7006:

    # ch_Name "{i}(I won't distract her.){/i}"
    ch_Name "{i}(Nem fogom zavarni őt.){/i}"

# game/logic/locations/ashlie_room.rpy:16
translate hungarian ashlie_room_label_02448681:

    # ch_Name "{i}(Although I could sneak a peek...){/i}"
    ch_Name "{i}(Habár benézhetek egy picit...){/i}"

# game/logic/locations/ashlie_room.rpy:42
translate hungarian ashlie_room_label_ad35e680:

    # ch_Name "{i}(Oh...){/i}"
    ch_Name "{i}(Oh...){/i}"

# game/logic/locations/ashlie_room.rpy:43
translate hungarian ashlie_room_label_0539956b:

    # ch_Name "{i}(Ashley in her finest - so graceful!){/i}"
    ch_Name "{i}(Ashely most nagyon jól néz ki - olyan bájos!){/i}"

# game/logic/locations/ashlie_room.rpy:44
translate hungarian ashlie_room_label_1e136241:

    # ch_Name "{i}(I could watch this forever...){/i}"
    ch_Name "{i}(Örökké tudnám ezt nézni...){/i}"

# game/logic/locations/ashlie_room.rpy:46
translate hungarian ashlie_room_label_58a2b055:

    # ch_Name "{i}(Alas, Don won't wait too long.){/i}"
    ch_Name "{i}(Nade, Don nem fog sokáig várni.){/i}"

# game/logic/locations/ashlie_room.rpy:47
translate hungarian ashlie_room_label_7f0478cb:

    # ch_Name "{i}(I'd better go.){/i}"
    ch_Name "{i}(Jobb, ha most megyek.){/i}"

# game/logic/locations/ashlie_room.rpy:71
translate hungarian ashlie_room_label_de7a2503:

    # ch_Name "{i}(Don's probably waiting for me in the living room.){/i}"
    ch_Name "{i}(Don valószínüleg a nappaliban vár engem.){/i}"

